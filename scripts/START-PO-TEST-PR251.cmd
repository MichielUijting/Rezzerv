...
setlocal
set "RZ_SELF=%~f0"
set "RZ_TMPPS=%TEMP%\Rezzerv-PO-test-PR251-%RANDOM%-%RANDOM%.ps1"

powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "$lines=[System.IO.File]::ReadAllLines($env:RZ_SELF); $i=[Array]::IndexOf($lines,'::REZZERV_POWERSHELL_PAYLOAD::'); if($i -lt 0){Write-Error 'Ingebouwde PowerShell-runner ontbreekt.'; exit 2}; [System.IO.File]::WriteAllLines($env:RZ_TMPPS,$lines[($i+1)..($lines.Length-1)],(New-Object System.Text.UTF8Encoding($false)))"
if errorlevel 1 (
  echo.
  echo LAUNCHER_ERROR: ingebouwde PowerShell-runner kon niet worden voorbereid.
...
if /I "%~1"=="-SelfTest" (
  powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%RZ_TMPPS%" -SelfTest
  set "EXITCODE=%ERRORLEVEL%"
  del /q "%RZ_TMPPS%" >nul 2>&1
  exit /b %EXITCODE%
)

echo Rezzerv PO-test PR #251 wordt gestart.
echo Vaste Rezzerv-werkmap: C:\Users\Gebruiker\Rezzerv_Github
echo Dit venster blijft na afloop of bij een fout open.
echo.

powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%RZ_TMPPS%"
set "EXITCODE=%ERRORLEVEL%"
del /q "%RZ_TMPPS%" >nul 2>&1

echo.
echo ============================================================
echo De PO-test is gestopt of afgerond.
echo Controleer hierboven TESTRESULTAAT en Cleanup afgerond.
echo Dit venster sluit pas nadat je een toets indrukt.
echo ============================================================
pause
exit /b %EXITCODE%
