#!/usr/bin/env python3
"""F7-05 Deep/Nightly dispatcher and exact-candidate aggregate gate."""
...
EXPECTED_BUCKETS = {"failure_recovery": 6, "legacy_portability": 6}


def die(message: str) -> None:
    raise SystemExit(f"FAIL F7-05: {message}")
...
def load_config(path: str) -> dict:
    cfg = load_json(ROOT / path)
    req(cfg.get("schema_version") == 1, "schema_version must be 1")
    req(cfg.get("gate_id") == "F7-05", "gate_id must be F7-05")
    req(cfg.get("target_ref") == "main", "target_ref must be main")
    return cfg


def validate_config(cfg: dict) -> None:
    workflows = cfg.get("workflows")
    req(isinstance(workflows, list) and len(workflows) == 12, "expected twelve Deep/Nightly workflows")
    req(cfg.get("required_workflow_count") == 12, "required_workflow_count drift")
    req(cfg.get("buckets") == EXPECTED_BUCKETS, "Deep/Nightly bucket counts drift")
    req(cfg.get("schedule_utc") == "30 1 * * *", "canonical nightly schedule drift")
    ids = [row.get("id") for row in workflows]
    req(len(ids) == len(set(ids)), "workflow ids must be unique")
    actual_buckets = {name: 0 for name in EXPECTED_BUCKETS}
    for row in workflows:
        ...
        req(bucket in EXPECTED_BUCKETS, f"invalid bucket: {wid}")
    ...
    req(actual_buckets == EXPECTED_BUCKETS, "actual bucket membership drift")
    print("PASS f7_05_twelve_deep_authorities_registered")
    print("PASS f7_05_six_failure_recovery_authorities_registered")
    print("PASS f7_05_six_legacy_portability_authorities_registered")
    print("PASS f7_05_all_children_manual_dispatchable")
    print("F7_05_DEEP_NIGHTLY_CONFIG_GREEN")
...
def api_request(method: str, url: str, token: str, payload: dict | None = None) -> dict | None:
    ...
    request = urllib.request.Request(url, data=data, method=method, headers={
        ...
        "User-Agent": "rezzerv-f7-deep-nightly-gate",
    })
    try:
...
def cmd_dispatch(args: argparse.Namespace) -> int:
    ...
    for row in cfg["workflows"]:
        ...
        evidence["workflows"].append({
            "id": row["id"], "bucket": row["bucket"], "workflow_file": workflow,
            "requested_inputs": row.get("inputs") or {}, "status": "dispatched",
        })
        print(f"F7_DEEP_DISPATCHED={row['id']}:{Path(workflow).name}")
    evidence["status"] = "dispatched"
    Path(args.evidence).write_text(json.dumps(evidence, indent=2) + "\n", encoding="utf-8")
    print(f"F7_DEEP_DISPATCHED_COUNT={len(evidence['workflows'])}")
    print("F7_DEEP_DISPATCH_GREEN")
    return 0
...
def latest_run(repo: str, workflow_file: str, ref: str, sha: str, not_before: datetime, token: str) -> dict | None:
    ...
    for run in data.get("workflow_runs", []):
        if run.get("event") != "workflow_dispatch" or run.get("head_sha") != sha:
            continue
        created = str(run.get("created_at") or "")
        if created and parse_time(created) >= not_before:
...
def cmd_wait(args: argparse.Namespace) -> int:
    ...
    deadline = time.monotonic() + int(cfg.get("timeout_seconds", 18000))
    ...
    while len(resolved) < len(cfg["workflows"]):
        ...
        for row in cfg["workflows"]:
            ...
            if not run:
                ...
                continue
            status, conclusion = str(run.get("status") or ""), run.get("conclusion")
            print(f"F7_DEEP_RUN {wid} run={run.get('id')} status={status} conclusion={conclusion}")
            if status != "completed":
            ...
            if conclusion != "success":
                ...
                continue
            resolved[wid] = {
                "id": wid, "bucket": row["bucket"], "workflow_file": row["workflow_file"],
                "run_id": run.get("id"), "head_sha": run.get("head_sha"),
                "conclusion": conclusion, "html_url": run.get("html_url"),
            }
        if len(resolved) == len(cfg["workflows"]):
            break
        if time.monotonic() >= deadline:
            die("timeout waiting for " + ",".join(waiting))
        print("F7_DEEP_WAITING=" + ",".join(waiting))
        time.sleep(poll)
    evidence["aggregate_runs"] = [resolved[row["id"]] for row in cfg["workflows"]]
    evidence["status"] = "green"
    evidence_path.write_text(json.dumps(evidence, indent=2) + "\n", encoding="utf-8")
    print(f"F7_DEEP_AGGREGATED_COUNT={len(resolved)}")
    print("F7_DEEP_NIGHTLY_GATE_GREEN")
    return 0
...
def main() -> int:
    ...
    validate.add_argument("--config", default="quality/ci/f7_deep_nightly_gate.json")
    validate.set_defaults(func=cmd_validate)
    dispatch = sub.add_parser("dispatch")
    dispatch.add_argument("--config", default="quality/ci/f7_deep_nightly_gate.json")
    dispatch.add_argument("--repo", required=True)
    dispatch.add_argument("--ref", required=True)
    dispatch.add_argument("--sha", required=True)
    dispatch.add_argument("--evidence", default="f7-deep-nightly-evidence.json")
    dispatch.set_defaults(func=cmd_dispatch)
    wait = sub.add_parser("wait")
    wait.add_argument("--config", default="quality/ci/f7_deep_nightly_gate.json")
    wait.add_argument("--repo", required=True)
    wait.add_argument("--ref", required=True)
    wait.add_argument("--sha", required=True)
    wait.add_argument("--evidence", default="f7-deep-nightly-evidence.json")
