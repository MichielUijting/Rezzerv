...
def cmd_plan(args: argparse.Namespace) -> int:
    ...
    for cluster in cfg["clusters"]:
        ...
        selected_for_risk = bool(matched) and args.risk_level != "S"
        planned.append({
            **cluster,
            "selected": selected_for_risk,
            "matched_paths": matched,
            "selection_reason": (
                "risk_level_S_cheap_only"
                if matched and args.risk_level == "S"
                else ("candidate_delta_match" if matched else "no_candidate_delta_match")
            ),
        })
