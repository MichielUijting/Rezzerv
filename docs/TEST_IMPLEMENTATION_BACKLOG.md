...
| F0-02 | P0 | Structurele validator + CI-gate invoeren | **Gereed op branch** | validator + workflow |
| F0-03 | P0 | Alle huidige P0 evidence inhoudelijk verifiëren en `inventory` omzetten naar `verified` of expliciet `gap` | **Bezig** | alle P0-scenario's beoordeeld |
| F0-04 | P0 | Historische PO-defectklassen aan permanente scenario's koppelen | Open | locatiebeleid, quantity precision, error feedback, article history, idempotentie e.d. zichtbaar in matrix |
| F0-05 | P0 | False confidence inventariseren: mocks, alleen compilechecks, alleen HTTP-status, SQLite-approximatie, UI zonder DB-eindstaat | Open | gap-analyse per scenario |
...
Fase 0 is klaar wanneer alle P0-scenario's inhoudelijk zijn geverifieerd en er geen impliciete aanname meer bestaat dat een workflownaam gelijkstaat aan functionele dekking.
