...
function ReceiptLivePreviewImage({ src, mode }) {
  ...
  if (!src) return null
  if (mode === 'original') {
    return <img src={src} alt="Live preview van kassabon" style={{ display: 'block', width: '100%', maxWidth: '100%', height: 'auto', background: '#fff', borderRadius: '4px' }} />
  }

  return <canvas ref={canvasRef} style={{ display: 'block', width: '100%', maxWidth: '100%', height: 'auto', background: '#fff', borderRadius: '4px' }} />
}

function ReceiptLivePreviewCard({ previewUrl, previewMode, onPreviewModeChange, visible }) {
    ...
    <ScreenCard>
      <div style={{ display: 'grid', gap: '16px' }}>
        ...
        <div style={{ display: 'grid', gap: '8px', maxWidth: '220px' }}>
          <div style={{ fontSize: '13px', fontWeight: 700, color: '#667085' }}>Weergave</div>
          <select value={previewMode} onChange={(event) => onPreviewModeChange(event.target.value)} style={{ width: '100%', border: '1px solid #d0d5dd', borderRadius: '8px', padding: '8px 12px', background: '#fff', fontSize: '14px' }}>
            <option value="original">Origineel</option>
            <option value="normalized">Genormaliseerd</option>
            <option value="ocr_ready">OCR-ready</option>
          </select>
        </div>

        <div style={{ border: '1px solid #d0d5dd', borderRadius: '8px', minHeight: '420px', background: '#f8fafc', overflow: 'auto', display: 'block', padding: '16px', height: '72vh', maxHeight: '72vh' }}>
          <ReceiptLivePreviewImage src={previewUrl} mode={previewMode} />
        </div>
      </div>
    </ScreenCard>
  )
...
function ReceiptProcessingDetailView({ previewUrl, previewMode, onPreviewModeChange, uploadProgress }) {
  return (
    <div style={{ display: 'grid', gap: '16px', gridTemplateColumns: 'minmax(0, 1fr) minmax(0, 1fr)', alignItems: 'start', width: '100%', maxWidth: '900px', margin: '0 auto', minWidth: 0, overflow: 'visible' }}>
      <div style={{ minWidth: 0, width: '100%', overflow: 'visible' }}>
        <ReceiptLivePreviewCard previewUrl={previewUrl} previewMode={previewMode} onPreviewModeChange={onPreviewModeChange} visible={Boolean(previewUrl)} />
      </div>
      <div style={{ minWidth: 0, width: '100%', overflow: 'visible' }}>
        <ScreenCard>
          <div style={{ display: 'grid', gap: '16px' }}>
            <div>
              <div style={{ fontWeight: 700, fontSize: '22px' }}>Bon wordt verwerkt</div>
              <div style={{ color: '#667085', marginTop: '4px' }}>De live preview blijft zichtbaar terwijl Rezzerv de bon inleest.</div>
            </div>
            <div style={{ display: 'grid', gap: '8px' }}>
              <div style={{ fontSize: '13px', fontWeight: 700, color: '#667085' }}>Voortgang</div>
              <div style={{ border: '1px solid #d0d5dd', borderRadius: '8px', background: '#f8fafc', padding: '16px' }}>
                <div style={{ fontWeight: 700 }}>{uploadProgress?.label || 'Bon verwerken…'}</div>
                <div style={{ color: '#667085', marginTop: '4px' }}>{uploadProgress?.message || 'Rezzerv verwerkt de geselecteerde bon.'}</div>
                <div style={{ marginTop: '12px', height: '10px', borderRadius: '999px', background: '#eaecf0', overflow: 'hidden' }}>
                  <div style={{ width: `${Math.max(0, Math.min(100, Number(uploadProgress?.percent || 0)))}%`, height: '100%', background: '#12B76A' }} />
                </div>
              </div>
            </div>
          </div>
...
export default function KassaPage() {
  ...
  const [uploadPreviewMode, setUploadPreviewMode] = useState('original')
  ...
  useEffect(() => {
    return () => {
      if (uploadPreviewUrl) window.URL.revokeObjectURL(uploadPreviewUrl)
    }
  }, [uploadPreviewUrl])
  ...
  function setLivePreviewFromFile(file) {
    ...
    const fileType = String(file.type || '').toLowerCase()
    const fileName = String(file.name || '').toLowerCase()
    const isImage = fileType.startsWith('image/') || fileName.endsWith('.jpg') || fileName.endsWith('.jpeg') || fileName.endsWith('.png') || fileName.endsWith('.webp')
    if (!isImage) {
      setUploadPreviewVisible(false)
      return
    }
    ...
    <AppShell title={isAddReceiptRoute ? 'Bon toevoegen' : 'Kassa'} showExit={false}>
      ...
      {isAddReceiptRoute ? (
        ...
        <div style={{ display: 'grid', gap: '16px' }} data-testid="kassa-page">
          ...
          {openedReceipt ? <ReceiptDetailView receipt={openedReceipt} canEdit={['admin','lid'].includes(currentUserDisplayRole)} onReceiptUpdated={(updated) => { setOpenedReceipt(updated); loadReceipts(householdId, { openReceiptId: updated?.id || openedReceiptId, prefetchedDetail: updated }).catch(() => {}) }} onFeedback={(variant, message) => { if (variant === 'error') { setError(message); setStatus('') } else { setStatus(message); setError('') } }} /> : uploadPreviewVisible && uploadPreviewUrl ? <ReceiptProcessingDetailView previewUrl={uploadPreviewUrl} previewMode={uploadPreviewMode} onPreviewModeChange={setUploadPreviewMode} uploadProgress={uploadProgress} /> : null}
        </div>
      )}
