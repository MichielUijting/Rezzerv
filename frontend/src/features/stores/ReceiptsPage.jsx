...
export default function ReceiptsPage() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      try {
        ...
        const response = await fetchJson(`/api/unpack-batches?householdId=${encodeURIComponent(householdData.id)}`)
        if (cancelled) return
  ...
  const listItems = useMemo(() => {
    return (batches || [])
      .map((batch) => ({
        ...batch,
        providerName: batch.store_name || batch.store_provider_name || 'Winkel',
        dateLabel: formatDateLabel(batch.purchase_date || batch.created_at),
        totalLines: Number(batch.line_count || batch.summary?.total || 0),
        statusLabel: batch.receipt_status || 'Nieuw',
      }))
                ...
                {isLoading ? (
                  ...
                  return (
                    <tr
                      ...
                      style={{
                        cursor: 'pointer',
                        boxShadow: `inset 4px 0 0 ${statusAccentColor(item.statusLabel)}`,
                      }}
                    >
