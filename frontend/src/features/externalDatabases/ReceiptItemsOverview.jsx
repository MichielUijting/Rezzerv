...
export default function ReceiptItemsOverview({ onError, onMessage }) {
  ...
  const selectedCandidates = linkedSelectedCandidate ? [linkedSelectedCandidate] : limitSearchCandidates(offSearchResults).map((result) => {
    ...
    return {
      ...
      status: catalogLinked
        ? 'Gekoppeld'
        : (result?.identity_compatible === false ? 'Merk wijkt af' : 'OFF-kandidaat'),
      hasUniversalCode: true,
      isLinkedToCatalog: catalogLinked,
      catalogLinked,
      identityCompatible: result?.identity_compatible !== false,
      isLinkableToCatalog: !catalogLinked && result?.identity_compatible !== false,
  ...
  const selectedCandidateCanBeLinked = Boolean(selectedItem && selectedCandidate && hasValidProductTypeDecision && hasValidProductTypeSource && !isLinkingProductType && !isLinkingGenericProduct && !isClassifyingProductType)
