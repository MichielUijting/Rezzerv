...
function SortableHeader({ field, label, sort, onSort }) {
  ...
  return (
    <th aria-sort={active ? (sort.direction === 'asc' ? 'ascending' : 'descending') : 'none'}>
      ...
      >
        <span>{label}</span>
        {indicator ? <span aria-hidden="true" data-testid={`sort-indicator-${field}`} style={{ marginLeft: 'auto', paddingRight: 8 }}>{indicator}</span> : null}
      </button>
    </th>
  )
}
...
export default function ShoppingPage() {
  ...
  const selectedItems = useMemo(() => (list.items || []).filter((item) => selectedItemIds.includes(item.id)), [list.items, selectedItemIds])
  const allVisibleSelected = visibleItems.length > 0 && visibleItems.every((item) => selectedItemIds.includes(item.id))
  ...
  async function updateItem(item, patch) {
    ...
    try {
      await requestJson(`/api/shopping-list/items/${encodeURIComponent(item.id)}`, { method: 'PUT', body: JSON.stringify(patch) })
      await loadList()
    } catch (saveError) {
        ...
        <Card>
          <div style={{ display: 'grid', gap: 18, width: '100%' }} data-testid="shopping-page">
            ...
            <Table dataTestId="shopping-list-table" resizableColumns tableStyle={tableStyle}>
              <colgroup><col style={{ width: 60 }} /><col style={{ width: 330 }} /><col style={{ width: 300 }} /><col style={{ width: 120 }} /><col style={{ width: 220 }} /><col style={{ width: 90 }} /></colgroup>
              <thead>
                ...
                <tr>
                  <th style={{ textAlign: 'center', verticalAlign: 'middle' }}><input type="checkbox" checked={allVisibleSelected} onChange={(event) => toggleAllVisible(event.target.checked)} aria-label="Selecteer alle zichtbare rijen" style={CHECKBOX_STYLE} /></th>
                  <th><input className="rz-input" style={FILTER_CONTROL_STYLE} value={filters.article} onChange={(event) => setFilters((current) => ({ ...current, article: event.target.value }))} placeholder="Zoeken" aria-label="Zoeken in winkellijst" /></th>
                  <th><select className="rz-input" style={FILTER_CONTROL_STYLE} value={filters.productType} onChange={(event) => setFilters((current) => ({ ...current, productType: event.target.value }))} aria-label="Filter producttype"><option value="">Filter</option>{productTypeOptions.map((value) => <option key={value} value={value}>{value}</option>)}</select></th>
                  <th>&nbsp;</th>
                  <th>&nbsp;</th>
                  <th style={{ textAlign: 'center', verticalAlign: 'middle' }}><input type="checkbox" checked={filters.checked === 'checked'} onChange={(event) => setFilters((current) => ({ ...current, checked: event.target.checked ? 'checked' : 'all' }))} aria-label="Filter gekocht" style={CHECKBOX_STYLE} /></th>
                </tr>
              </thead>
              <tbody>
                {loading ? <tr><td colSpan={6}>Winkellijst laden…</td></tr> : visibleItems.length === 0 ? <><tr><td colSpan={6}>Nog geen artikelen op de winkellijst.</td></tr><tr><td colSpan={6}>&nbsp;</td></tr><tr><td colSpan={6}>&nbsp;</td></tr></> : visibleItems.map((item) => (
                  <tr key={item.id}>
                    <td style={{ textAlign: 'center' }}><input type="checkbox" checked={selectedItemIds.includes(item.id)} onChange={(event) => toggleSelectedItem(item.id, event.target.checked)} aria-label={`Selecteer ${item.article_name}`} style={CHECKBOX_STYLE} /></td>
                    <td title={item.article_name}>{item.article_name}</td>
                    <td title={item.product_type_name}>{item.product_type_name}</td>
                    <td><input className="rz-input" style={inlineInputStyle} defaultValue={item.size || ''} aria-label={`Omvang ${item.article_name}`} onBlur={(event) => updateItem(item, { size: event.target.value })} /></td>
                    ...
                    <td style={{ textAlign: 'center' }}><input type="checkbox" checked={Boolean(item.checked)} onChange={(event) => updateChecked(item, event.target.checked)} aria-label={`Gekocht ${item.article_name}`} style={CHECKBOX_STYLE} /></td>
                  </tr>
                ))}
              </tbody>
            </Table>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12 }}>
              <Button type="button" onClick={deleteSelectedItems} disabled={saving || selectedItems.length === 0}>Verwijderen</Button>
              <Button type="button" onClick={exportSelectedItems} disabled={selectedItems.length === 0}>Exporteren</Button>
            </div>
          </div>
        </Card>
