...
async function prepareLayer1ReceiptFixture(frame, fixture) {
  if (fixture.batchId && fixture.completeLineId && fixture.incompleteLineId) {
    ...
    const validated = await validateReceiptFixtureSelection(frame, resolved, fixture)
    if (validated) {
      persistLayer1ReceiptFixture(validated, fixture)
      frame.__rezzervLayer1ReceiptFixture = validated
      return validated
    }
  }
  ...
  try {
    ...
    persistLayer1ReceiptFixture(resolved, fixture)
...
function persistLayer1ReceiptFixture(resolved, fixture = {}) {
  const nextFixture = {
  ...
  try {
    window.localStorage.setItem('rezzerv_layer1_fixture', JSON.stringify(nextFixture))
  } catch {
    // negeer opslagfouten
  }
  return nextFixture
...
async function validateReceiptFixtureSelection(frame, resolved, fixture) {
  try {
    const { detailDoc, batchId } = await openReceiptBatchWithSelectableLines(frame, resolved.batchId, resolved.completeLineId)
    ...
    return {
      connectionId: String(resolved.connectionId || ''),
      latestBatchId: String(resolved.latestBatchId || batchId || resolved.batchId || ''),
      batchId: String(batchId || resolved.batchId || ''),
      completeLineId,
      incompleteLineId,
    }
  } catch {
...
export async function runLayer1RegressionTests() {
  ...
  try {
    ...
    await runScenario('T6 Complete kassabonregel kan naar voorraad', async () => {
      ...
      const detailDoc = await openReceiptDetail(frame, receiptFixture.batchId)
      await waitForReceiptLines(() => getFrameDocument(frame))
      const resolvedCompleteLineId = detailDoc.querySelector(`[data-testid="receipt-line-select-${receiptFixture.completeLineId}"]`)
        ? String(receiptFixture.completeLineId)
        : findReceiptLineIdByLabel(detailDoc, fixture.completeLineLabel)
      if (!resolvedCompleteLineId) throw new Error('Layer1 receipt fixture ontbreekt of is incompleet')
      const normalizedReceiptFixture = {
        ...receiptFixture,
        completeLineId: String(resolvedCompleteLineId),
      }
      persistLayer1ReceiptFixture(normalizedReceiptFixture, fixture)
      frame.__rezzervLayer1ReceiptFixture = normalizedReceiptFixture
      const lineSelect = detailDoc.querySelector(`[data-testid="receipt-line-select-${resolvedCompleteLineId}"]`)
    ...
    await runScenario('T7 Incomplete kassabonregel wordt geblokkeerd', async () => {
      ...
      const detailDoc = await openReceiptDetail(frame, receiptFixture.batchId)
      await waitForReceiptLines(() => getFrameDocument(frame))
      const resolvedIncompleteLineId = detailDoc.querySelector(`[data-testid="receipt-line-select-${receiptFixture.incompleteLineId}"]`)
        ? String(receiptFixture.incompleteLineId)
        : findReceiptLineIdByLabel(detailDoc, fixture.incompleteLineLabel)
      if (!resolvedIncompleteLineId) throw new Error('Layer1 receipt fixture ontbreekt of is incompleet')
      const normalizedReceiptFixture = {
        ...receiptFixture,
        incompleteLineId: String(resolvedIncompleteLineId),
      }
      persistLayer1ReceiptFixture(normalizedReceiptFixture, fixture)
      frame.__rezzervLayer1ReceiptFixture = normalizedReceiptFixture
      const lineSelect = detailDoc.querySelector(`[data-testid="receipt-line-select-${resolvedIncompleteLineId}"]`)
      if (!lineSelect) throw new Error('Layer1 receipt fixture ontbreekt of is incompleet')
      const lineId = extractIdFromTestId(lineSelect, 'receipt-line-select-')
    ...
    try {
      await requestJson('/api/dev/regression/cleanup', { method: 'POST', body: '{}' })
    } catch {}
    removeExistingFrame()
  }
