...
async function cleanupRegressionFixtures() {
  try {
    await requestJson('/api/dev/regression/cleanup', { method: 'POST', body: '{}' })
  } catch {
    // cleanup is best effort en mag de rapportage niet blokkeren
  }
}

async function prepareRegressionFixture(frame, options = {}) {
  const { forceReset = true } = options
  setRegressionStep(forceReset ? 'Fixture veilig resetten' : 'Fixture controleren')

  const prepared = await requestJson('/api/dev/browser-regression/reset-fixture', { method: 'POST', body: '{}' })
  const deterministicArticles = Array.isArray(prepared?.deterministic_articles) ? prepared.deterministic_articles : []
  const fixture = {
    pantryId: prepared?.pantry_id || null,
    shelfId: prepared?.shelf_id || null,
    kitchenId: prepared?.kitchen_id || null,
    workbenchId: prepared?.workbench_id || null,
    deterministicArticles: deterministicArticles.map((item) => item.naam),
    dataset: prepared?.dataset || 'browser_regression_fixture',
  }

  try {
    console.info('[Rezzerv regressie] browser-fixture', prepared)
  } catch {}
  ...
  await waitForAsyncCondition(async () => {
    ...
    return deterministicArticles.every((item) => names.includes(String(item?.naam || '').toLowerCase()))
...
export async function runBrowserRegressionTests() {
  ...
  try {
    ...
    await runScenario('Winkelwaarschuwing Terug annuleert verwerking zonder voorraadeffect', async () => {
      ...
      if (!String(frame.contentWindow?.location?.pathname || '').startsWith('/kassabonnen/batch/')) {
    ...
    await cleanupRegressionFixtures()
