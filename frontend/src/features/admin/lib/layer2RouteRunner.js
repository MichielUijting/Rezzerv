...
async function openReceiptDetailWithSelectableLines(frame, preferredBatchId = null) {
  ...
  for (const batchId of candidateIds) {
    ...
    const lineSelect = detailDoc?.querySelector('[data-testid^="receipt-line-select-"]')
...
export async function runLayer2RouteTests() {
  ...
  try {
    ...
    await runScenario('R12 Kassabondetail selectie en exportflow werkt', async ()=>{ const { detailDoc, lineSelect } = await openReceiptDetailWithSelectableLines(frame, fixture.batchId); const exportButton=detailDoc.querySelector('[data-testid="receipt-export-button"]'); if(!lineSelect||!exportButton) throw new Error('Kassabondetail export ontbreekt in routeflow'); if(!lineSelect.checked) clickElement(lineSelect); await delay(160); const activeExportButton=getFrameDocument(frame)?.querySelector('[data-testid="receipt-export-button"]')||exportButton; if(activeExportButton.disabled) throw new Error('Kassabondetail export wordt niet actief in routeflow'); clickElement(activeExportButton); await delay(220); if(!getLastDownload(frame)?.csv) throw new Error('Kassabondetail export ontbreekt in routeflow') }, results)
