...
export default function AdminPage() {
  ...
  async function handleRunProcessDiagnostic() {
    ...
    try {
      const res = await fetch(`/api/dev/diagnostics/store-process-validation?householdId=${encodeURIComponent(householdId)}`, { headers: getAuthHeaders() });
      const data = await res.json();
      ...
      const suites = [
        {
          ...
          successMessage: 'Regressietest alles: laag 1 afgerond',
          failureMessage: 'Laag-1 kernregressietest kon niet worden uitgevoerd',
          prefix: 'Laag 1',
        },
        {
          ...
          successMessage: 'Regressietest alles: laag 2 afgerond',
          failureMessage: 'Laag-2 route-/schermtest kon niet worden uitgevoerd',
          prefix: 'Laag 2',
        },
        {
          ...
          successMessage: 'Regressietest alles: laag 3 afgerond',
          failureMessage: 'Laag-3 UI/styleguide-test kon niet worden uitgevoerd',
          prefix: 'Laag 3',
        },
      ];
      ...
      for (const suite of suites) {
        try {
          const results = await executeLayerTest({
            layerId: suite.layerId,
            startFn: suite.startFn,
            runnerFn: suite.runnerFn,
            startMessage: suite.startMessage,
            successMessage: suite.successMessage,
          ...
          if (!error?.alreadyRunning) {
            try {
              await submitTestResults(suite.layerId, [{
  ...
  async function handleRunAlmostOutSelfTest() {
    setTestMessage('');
    setShowReport(false);
    try {
      const report = await runAlmostOutSelfTest();
      await refreshTestStatus();
      setTestReport(report);
      setShowReport(true);
      const failedCount = report?.failed_count || report?.results?.filter((result) => result.status === 'failed').length || 0;
      const blockedCount = report?.blocked_count || report?.results?.filter((result) => result.status === 'blocked').length || 0;
      setTestMessage(
        failedCount > 0
          ? `Almost-out backend self-test afgerond: ${failedCount} fout(en)${blockedCount ? ` en ${blockedCount} geblokkeerd` : ''}.`
          : `Almost-out backend self-test geslaagd.${blockedCount ? ` ${blockedCount} scenario('s) bewust geblokkeerd voor inconsistentiecontrole.` : ''}`
      );
    } catch (error) {
      setTestMessage(error.message || 'Almost-out backend self-test kon niet worden gestart');
    }
  }
        ...
        <div className="rz-admin-grid">
          ...
          <div className="rz-admin-panel">
            ...
            <div data-testid="test-run-panel"><TestRunPanel
              ...
              onRunAlmostOutSelfTest={handleRunAlmostOutSelfTest}
            ...
            {showReport && testReport ? (
              <div className="rz-admin-report">
                ...
                <div className="rz-admin-report-meta">
                  ...
                  <div>Geblokkeerd: {testReport.blocked_count || testReport.results?.filter((result) => result.status === "blocked").length || 0}</div>
                  {testReport.test_type === 'almost_out_self_test' ? <div data-testid="almost-out-report-hint">PO-check almost-out: geslaagd als Gefaald = 0. Geblokkeerd hoort alleen bij de bewuste inconsistentiecontrole.</div> : null}
                </div>
                <div className="rz-admin-report-list">
                  {testReport.results?.length ? testReport.results.map((result) => (
                    <div key={result.name} className={`rz-admin-report-row rz-admin-report-row--${result.status}`}>
                      ...
                      {result.details?.expected_almost_out !== undefined ? <div className="rz-admin-report-meta-line">Almost-out: verwacht {String(result.details.expected_almost_out)} · gevonden {String(result.details.actual_almost_out)}</div> : null}
                      {result.details?.expected_total_quantity !== undefined ? <div className="rz-admin-report-meta-line">Voorraad: verwacht {result.details.expected_total_quantity} · gevonden {result.details.actual_total_quantity}</div> : null}
                      {result.details?.notes ? <div className="rz-admin-report-meta-line">Toelichting: {result.details.notes}</div> : null}
