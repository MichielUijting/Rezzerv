...
function Feedback({ type = 'info', children }) {
  ...
  const color = isError ? '#991b1b' : '#005F6A'
      ...
      <div className="rz-modal-card" role="dialog" aria-modal="true" aria-labelledby="sublocations-modal-title">
        ...
        <div style={{ display: 'grid', gap: 16 }}>
          ...
          <label style={{ display: 'flex', alignItems: 'center', gap: 10, color: '#0f172a', fontWeight: 600 }}>
            <input type="checkbox" style={{ accentColor: '#005F6A', width: 16, height: 16 }} checked={Boolean(form.active)} onChange={(event) => onChange({ ...form, active: event.target.checked })} />
            Actief
          </label>
        </div>
          ...
          <Table wrapperClassName="rz-stock-table-wrapper" tableClassName="rz-stock-table" dataTestId="settings-sublocations-table" tableStyle={{ tableLayout: 'fixed', width: buildTableWidth(columnWidths), minWidth: buildTableWidth(columnWidths) }}>
              ...
              <thead>
                <tr className="rz-table-header">
                  <th><input type="checkbox" style={{ accentColor: '#005F6A', width: 16, height: 16 }} checked={allFilteredSelected} onChange={toggleAllFiltered} aria-label="Selecteer alle zichtbare sublocaties" /></th>
                  <th>Sublocatie</th>
                  <th>Ruimte</th>
                  ...
                  <th className="rz-num">
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, justifyContent: 'center', minHeight: 20, width: '100%' }}>
                      <label style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 12 }}>
                        <input type="checkbox" style={{ accentColor: '#005F6A', width: 16, height: 16 }} checked={filters.actiefJa} onChange={(event) => setFilters((current) => ({ ...current, actiefJa: event.target.checked }))} />Ja
                      </label>
                      <label style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 12 }}>
                        <input type="checkbox" style={{ accentColor: '#005F6A', width: 16, height: 16 }} checked={filters.actiefNee} onChange={(event) => setFilters((current) => ({ ...current, actiefNee: event.target.checked }))} />Nee
                      </label>
                    </div>
                  </th>
                </tr>
                ...
                {isLoading ? (
                  ...
                  return (
                    <tr key={item.id} className={selected ? 'rz-row-selected' : ''}>
                      <td><input type="checkbox" style={{ accentColor: '#005F6A', width: 16, height: 16 }} checked={selected} onChange={() => toggleSelected(item.id)} aria-label={`Selecteer ${item.naam}`} /></td>
                      <td>
                      ...
                      <td className="rz-num"><input type="checkbox" style={{ accentColor: '#005F6A', width: 16, height: 16 }} checked={Boolean(item.active)} readOnly aria-label={`Actief ${item.naam}`} /></td>
                    </tr>
                  )
                })}
              </tbody>
