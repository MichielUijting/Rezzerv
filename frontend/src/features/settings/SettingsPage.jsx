...
export default function SettingsPage() {
  ...
  function getTileStyle(disabled = false) {
    return {
      ...
      boxShadow: disabled ? 'none' : undefined, opacity: 1,
    }
  }

  function handleDisabledClick(event) { event.preventDefault() }

  const adminOnlyProps = {
    style: getTileStyle(!isAdmin),
    'aria-disabled': isAdmin ? 'false' : 'true',
      ...
      <Card>
        <div style={{ display: 'grid', gap: '18px' }} data-testid="settings-page">
          ...
          <Link to="/instellingen/artikelgroepen" style={getTileStyle(isViewer)} aria-disabled={isViewer ? 'true' : 'false'} onClick={isViewer ? handleDisabledClick : undefined}>
            <div><div style={{ fontWeight: 600 }}>Artikelgroepen</div><div style={{ color: isViewer ? '#0f5b32' : '#667085', fontSize: '14px' }}>{isViewer ? 'Alleen Admin kan Artikelgroepen beheren.' : 'Beheer je eigen indeling van voorraadartikelen'}</div></div><div aria-hidden="true">→</div>
          </Link>
          ...
          <Link to="/instellingen/locaties" {...adminOnlyProps}>
            <div><div style={{ fontWeight: 600 }}>Locaties</div><div style={{ color: !isAdmin ? '#0f5b32' : '#667085', fontSize: '14px' }}>{!isAdmin ? 'Alleen Admin kan dit scherm openen.' : 'Beheer locaties en sublocaties voor Voorraad, Kassa en Incidentele aankoop'}</div></div><div aria-hidden="true">→</div>
          </Link>
          <Link to="/instellingen/winkelimport" style={getTileStyle(isViewer)} aria-disabled={isViewer ? 'true' : 'false'} onClick={isViewer ? handleDisabledClick : undefined}>
            <div><div style={{ fontWeight: 600 }}>Winkelimport</div><div style={{ color: isViewer ? '#0f5b32' : '#667085', fontSize: '14px' }}>{isViewer ? 'Zichtbaar voor kijkers, maar alleen Artikeldetails is beschikbaar.' : 'Vereenvoudigingsniveau voor het huishouden · alleen beheerder kan wijzigen'}</div></div><div aria-hidden="true">→</div>
          </Link>
          <Link to="/instellingen/huishouden" style={getTileStyle(isViewer)} aria-disabled={isViewer ? 'true' : 'false'} onClick={isViewer ? handleDisabledClick : undefined}>
            <div><div style={{ fontWeight: 600 }}>Huishouden</div><div style={{ color: isViewer ? '#0f5b32' : '#667085', fontSize: '14px' }}>{isViewer ? 'Zichtbaar voor kijkers, maar alleen Artikeldetails is beschikbaar.' : 'Naam, leden en rollen beheren'}</div></div><div aria-hidden="true">→</div>
          </Link>
          ...
          <Link to="/instellingen/bijna-op-voorspelling" style={getTileStyle(isViewer)} aria-disabled={isViewer ? 'true' : 'false'} onClick={isViewer ? handleDisabledClick : undefined}>
            <div><div style={{ fontWeight: 600 }}>Bijna op voorspelling</div><div style={{ color: isViewer ? '#0f5b32' : '#667085', fontSize: '14px' }}>{isViewer ? 'Zichtbaar voor kijkers, maar alleen Artikeldetails is beschikbaar.' : 'Huishoudbrede almost-out voorspelling en regelprioriteit'}</div></div><div aria-hidden="true">→</div>
          </Link>
        </div>
      </Card>
    </AppShell>
