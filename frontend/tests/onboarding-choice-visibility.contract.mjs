...
const checks = [
  ...
  [waar.includes('Instellingen → Locaties'), 'Waar Inhuis directs location management to Settings'],
  [!waar.includes('<CheckboxChoice') && !waar.includes('main_locations') && !waar.includes('sublocations:'), 'Waar Inhuis onboarding contains no location-entry model'],
  [!waar.includes('waar-inhuis-custom-location-input') && !waar.includes('waar-inhuis-refine-locations') && !waar.includes('waar-inhuis-sublocation-add'), 'Waar Inhuis onboarding contains no location-entry controls'],
  [shared.includes('title="Jouw volledige inrichting"'), 'final household step reviews the complete setup'],
  [shared.includes("Kies eerst of je Inhuis alleen of samen gebruikt."), 'final household step explains the missing required usage choice'],
  [shared.includes('data-testid="shared-household-finish-validation"'), 'final household step exposes visible completion validation'],
  [shared.includes('disabled={saving || inviteSaving}'), 'final completion remains clickable so missing choices can produce feedback'],
  [shared.includes('Na afronden ga je direct naar Home.'), 'final household step explains successful navigation to Home'],
