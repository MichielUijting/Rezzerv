...
assert.ok(fs.existsSync(path.join(frontendRoot, 'public/inhuis-loading-mark.svg')))
assert.match(loadingMark, /stroke="#005F6A"/i)
assert.doesNotMatch(loadingMark, /<text\b/i)
assert.match(tableLoadingOverlay, /const DEFAULT_DELAY_MS = 1000/)
assert.match(tableLoadingOverlay, /src="\/inhuis-loading-mark\.svg"/)
assert.match(tableLoadingOverlay, /data-testid="table-loading-logo"/)
assert.match(tableLoadingCss, /width:\s*min\(420px,\s*80vw\)/)
