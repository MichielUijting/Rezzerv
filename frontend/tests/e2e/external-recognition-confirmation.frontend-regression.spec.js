...
test.describe('Externe herkenning bevestigen geïntegreerde regressie', () => {
  test('behoudt volwassen overzichtsfuncties en bevestigt herkenning in hetzelfde detail', async ({ page }) => {
    ...
    await page.route('**/api/external-products/off/search', async (route) => {
      ...
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          query: payload?.query || 'Veldsla herkenning regressietest',
          results: [],
        }),
      })
    })
    ...
    // Er is precies één hoofdlijst: de volwassen bonartikelentabel.
    ...
    // Standaard DataTable borgt sticky kop + filterregel generiek.
    ...
    // Bestaande paginering blijft functioneel met meer dan tien regels.
    ...
    // Bestaand zoeken/filteren in de hoofdtabel blijft aanwezig.
    const receiptFilter = table.getByRole('textbox', { name: /Filter op .*Bonartikel|Filter op Bonartikel/i }).first()
    ...
    // Het bestaande detail blijft de enige plek waar vervolgstappen plaatsvinden.
    ...
    // Herkenning bevestigen is in datzelfde detail geïntegreerd, los van Catalogus-koppeling.
    ...
    // Bevestigen mag de bestaande Catalogus-flow niet activeren of simuleren.
