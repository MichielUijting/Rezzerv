...
async function routeProductGroups(page) {
  ...
  await page.route('**/api/inventory/groups', async (route) => {
    await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ ok: true, items: [{ inventory_group_key: 'gpc:10000001', display_name: 'Mosterd extra lange productgroepnaam', gpc_family_name: 'Voeding', gpc_class_name: 'Sauzen', gpc_brick_code: '10000001', base_unit: 'stuk', item_count: assigned ? 2 : 1, products: [{ inventory_id: 'test-mosterd', product_name: 'Mosterd fijne Dijon extra lange artikelnaam', stock_quantity: 1 }, ...(assigned ? [{ inventory_id: 'test-boor', product_name: 'Boormachine met zeer lange artikelnaam', stock_quantity: 1 }] : [])] }], group_options: groupOptions, unresolved_items: assigned ? [] : [{ inventory_id: 'test-boor', product_name: 'Boormachine met zeer lange artikelnaam', stock_quantity: 1, reason: 'no_inventory_group_match' }], total_groups: 1, total_unresolved_items: assigned ? 0 : 1, source: 'inventory_group_projection_v2_gpc_hierarchy', mutates_inventory: false }) });
  });
  await page.route('**/api/product-groups', async (route) => {
    if (route.request().method() === 'POST') {
      const payload = JSON.parse(route.request().postData() || '{}');
      groupOptions.push({ inventory_group_key: 'productgroep.soep', display_name: payload.display_name || 'Soep', default_base_unit: 'stuk', gpc_family_name: payload.gpc_family_name || '', gpc_class_name: payload.gpc_class_name || '', gpc_brick_code: '' });
      await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ ok: true, inventory_group_key: 'productgroep.soep', display_name: payload.display_name || 'Soep', gpc_family_name: payload.gpc_family_name || '', gpc_class_name: payload.gpc_class_name || '', default_base_unit: 'stuk', mutates_inventory: false }) });
      return;
    }
  ...
  await page.route('**/api/product-groups/gpc%3A10000001', async (route) => {
    if (route.request().method() === 'PUT') {
      const payload = JSON.parse(route.request().postData() || '{}');
      groupOptions[0] = { inventory_group_key: 'gpc:10000001', display_name: payload.display_name || 'Mosterd bijgewerkt', default_base_unit: 'stuk', gpc_family_name: payload.gpc_family_name || 'Voeding', gpc_class_name: payload.gpc_class_name || 'Sauzen', gpc_brick_code: '10000001' };
      await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ ok: true, inventory_group_key: 'gpc:10000001', display_name: payload.display_name || 'Mosterd bijgewerkt', default_base_unit: 'stuk', mutates_inventory: false }) });
      return;
    }
...
test.describe('Productgroepen frontend-regressie', () => {
  test('Landingspagina opent beheergrid met selecteerbare GPC-hiërarchie en beheert niveaus', async ({ page }) => {
