...
async function ensureBackendPython() {
  ...
  let ready = false
  try { await runCommand(venvPython, ['-c', 'import fastapi, sqlalchemy, uvicorn'], { cwd: backendDir, env: { ...process.env } }, installLogPath); ready = true } catch { ready = false }
...
async function runRegressionBrowser() {
  ...
  try {
    ...
    await page.waitForSelector('[data-testid="regression-runner-status"][data-status="completed"]', { timeout: 300000 })
...
async function main() {
  ...
  try {
    ...
    report.overall_status = report.build.status === 'passed' && report.health.status === 'passed' && regression.overall_status === 'passed' ? 'passed' : 'failed'
