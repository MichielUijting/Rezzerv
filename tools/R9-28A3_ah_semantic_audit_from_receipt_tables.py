...
def classify(row: dict[str, Any]) -> SemanticLine:
    ...
    def make(line_type, article, financial, loyalty, count, article_sum, total_validation, rule, reason):
        return SemanticLine(**base, line_type=line_type, article_candidate=article, financial_effect=financial,
                            loyalty_effect=loyalty, include_in_article_count=count,
                            include_in_article_sum=article_sum, include_in_total_validation=total_validation,
    ...
    if match_any(n, PATTERNS["AH_TAX"]):
        return make("AH_TAX", False, "tax", False, False, False, False, "AH_TAX_RULE", "btw/fiscale regel")
    if match_any(n, PATTERNS["AH_LOYALTY_STAMPS"]):
        return make("AH_LOYALTY_STAMPS", False, "loyalty_amount" if db_line_total is not None or text_amount is not None else "none", True, False, False, db_line_total is not None or text_amount is not None, "AH_LOYALTY_STAMPS_RULE", "zegels bewaren, niet als artikel")
    ...
    if match_any(n, PATTERNS["AH_DISCOUNT"]):
        return make("AH_DISCOUNT", False, "discount" if db_line_total is not None or text_amount is not None else "discount_metadata", False, False, False, db_line_total is not None or text_amount is not None, "AH_DISCOUNT_RULE", "korting bewaren, niet als artikel")
...
def summarize(lines: list[SemanticLine]) -> dict[str, Any]:
    ...
    return {
        ...
        "article_sum_from_db_line_total": round(article_sum, 2),
...
def main() -> int:
    ...
    ap.add_argument("--out", default="/tmp/R9-28A3_reports")
    args = ap.parse_args()
    ...
    audit = {
        "audit": "R9-28A3 AH semantic line classification audit from receipt_tables/receipt_table_lines",
    ...
    path = out_dir / f"R9-28A3_ah_semantic_line_classification_audit_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    path.write_text(json.dumps(audit, indent=2, ensure_ascii=False), encoding="utf-8")

    print(f"R9-28A3 audit geschreven naar: {path}")
    print(f"receipt_count={len(receipts)} line_count={len(classified)}")
    for r in receipts:
        print(f"{r['receipt_table_id']} | {r.get('store_name')} | total={r.get('total_amount')} | reference={r.get('reference')} | {r['summary']}")
    return 0
