...
def write_markdown(items: list[ModuleInventoryItem]) -> None:
    lines: list[str] = []
    category_counts = Counter(item.repository_category for item in items)
    refactor_counts = Counter(item.refactor_status_guess for item in items)
    ...
    lines.append("")
    lines.append("## Repository categories")
    lines.append("")
    ...
    lines.append("")
    lines.append("## Refactor statuses")
    lines.append("")
    lines.append("| Refactor status | Count |")
    lines.append("|---|---:|")
    for status, count in sorted(refactor_counts.items()):
        lines.append(f"| {status} | {count} |")
    lines.append("")
    lines.append("## Inventory")
    lines.append("")
    lines.append("| Path | Category | Lines | TD Section | Runtime | Refactor | Routes | DB R/W | Header | Header scope |")
    lines.append("|---|---|---:|---|---|---|---:|---|---|---|")
    for item in items:
        db = f"{'R' if item.reads_data else '-'}{'W' if item.writes_data else '-'}"
        header = "yes" if item.has_technical_design_header else "no"
        scope = "yes" if item.header_scope_default else "no"
        lines.append(
            f"| `{item.path}` | {item.repository_category} | {item.line_count} | {item.td_section_guess} | {item.runtime_type_guess} | {item.refactor_status_guess} | {len(item.fastapi_routes)} | {db} | {header} | {scope} |"
        )
    lines.append("")
    ...
    for item in items:
        if item.status_terms:
            lines.append(f"- `{item.path}` ({item.repository_category}): {', '.join(item.status_terms)}")
    lines.append("")
    lines.append("## Modules with FastAPI routes")
    lines.append("")
    for item in items:
        if item.fastapi_routes:
            lines.append(f"### `{item.path}` ({item.repository_category})")
...
def main() -> None:
    ...
    for category, count in sorted(Counter(item.repository_category for item in items).items()):
