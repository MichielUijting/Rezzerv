...
s = s.replace(
    ...
    "_normalize_filename(b['source_file']) == _normalize_filename(r['source_file'])",
)
s = s.replace(
    "str(actual.get('source_file') or '') == str(expected.get('source_file') or '')",
    "_normalize_filename(actual.get('source_file')) == _normalize_filename(expected.get('source_file'))",
)
...
# cleanup workflow and temporary script after patch commit
try:
