...
def build_report(zip_path: Path, out_dir: Path, preprocess: bool, expected_ah_count: int | None, max_selected_diagnostics: int | None) -> dict[str, Any]:
    ...
    return {
        "audit": "R9-28B6C4 existing parser store detection batch with MIME type",
...
def render_markdown(report: dict[str, Any]) -> str:
    lines = [
        "# R9-28B6C4 — AH-selectie via bestaande winkelherkenning met MIME-type",
        "",
...
def main() -> int:
    ...
    parser.add_argument("--out", default="/tmp/R9-28B6C4_existing_store_batch")
    ...
    json_path = out_dir / f"R9-28B6C4_existing_store_detection_batch_{stamp}.json"
    md_path = out_dir / f"R9-28B6C4_existing_store_detection_batch_{stamp}.md"
    json_path.write_text(json.dumps(report, indent=2, ensure_ascii=False), encoding="utf-8")
    md_path.write_text(render_markdown(report), encoding="utf-8")

    print("R9-28B6C4 resource-safe existing parser store-detection batch geschreven:")
    print(f"- {json_path}")
    print(f"- {md_path}")
