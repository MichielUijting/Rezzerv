...
def _test_engine(tmp_path):
    return create_engine(f"sqlite:///{tmp_path / 'recognition-test.db'}", future=True)


def _prepare_database(tmp_path, monkeypatch):
    engine = _test_engine(tmp_path)
    monkeypatch.setattr(candidate_store, "engine", engine)
    monkeypatch.setattr(recognition, "engine", engine)
...
def test_confirm_recognition_does_not_create_catalog_or_inventory_data(tmp_path, monkeypatch):
    engine = _prepare_database(tmp_path, monkeypatch)
...
def test_recognition_confirmation_is_idempotent_and_requires_explicit_overwrite(tmp_path, monkeypatch):
    engine = _prepare_database(tmp_path, monkeypatch)
...
def test_confirmed_recognition_is_skipped_by_later_candidate_search(tmp_path, monkeypatch):
