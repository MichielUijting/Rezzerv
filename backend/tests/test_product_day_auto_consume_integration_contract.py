...
def test_auto_consume_decision_accepts_and_uses_purchase_date():
    ...
    function = next(
        ...
        and node.name == "determine_auto_consume_decision"
    )
...
def test_store_import_passes_receipt_purchase_date_to_auto_consume():
    tree = _main_tree()
    calls = [
        node
...
def test_existing_purchase_and_auto_repurchase_writes_keep_purchase_date():
    tree = _main_tree()
    functions = {
        node.name: node
        for node in tree.body
    ...
    for name in ("create_inventory_purchase_event", "create_auto_repurchase_event"):
