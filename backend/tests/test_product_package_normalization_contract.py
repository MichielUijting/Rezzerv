...
def test_single_package_becomes_one_purchased_unit_with_content():
    label, quantity, unit, metadata = apply_package_extraction_to_candidate('opaque-alpha 500g')
    assert label == 'opaque-alpha'
    assert quantity == 1
    assert unit == 'stuk'
...
def test_purchase_count_is_not_overwritten_by_package_content():
    label, quantity, metadata = normalize_product_name_label('2 x opaque-beta 500g')
    assert label == 'opaque-beta 500g'
    assert quantity == 2
    label, quantity, unit, package = apply_package_extraction_to_candidate(label, quantity=quantity)
    assert label == 'opaque-beta'
    assert quantity == 2
    assert unit is None
...
def test_trailing_purchase_count_is_removed_with_explicit_transaction_proof():
    label, quantity, metadata = normalize_product_name_label(
        'opaque-gamma Q7 2',
        transaction_text='opaque-gamma Q7 2 x 4,49 8,98',
    )
    assert label == 'opaque-gamma Q7'
    assert quantity == 2
...
def test_trailing_purchase_count_is_removed_with_financial_proof_when_parser_already_split_multiplier():
    label, quantity, metadata = normalize_product_name_label(
        'opaque-delta Q7 2',
        transaction_text='opaque-delta Q7 2',
        unit_price='4,49',
        line_total='8,98',
    )
    assert label == 'opaque-delta Q7'
    assert quantity == 2
    assert 'trailing_transaction_item_count_removed' in metadata['normalization_rules']
...
def test_trailing_product_numbers_are_preserved_without_matching_transaction_or_financial_proof():
    cases = (
        ('opaque-epsilon B12', '4,99', '4,99'),
        ('opaque-zeta 16', '4,99', '4,99'),
        ('opaque-eta 2', '4,99', '4,99'),
        ('opaque-theta Q1', '4,99', '4,99'),
    )
...
def test_financial_proof_must_match_trailing_count_exactly():
    normalized, quantity, metadata = normalize_product_name_label(
        'opaque-iota 2',
        transaction_text='opaque-iota 2',
        unit_price='4,99',
        line_total='14,97',
    )
    assert normalized == 'opaque-iota 2'
    assert quantity is None
    assert metadata is None


def test_multipack_is_structured_without_leaking_into_name():
    result = extract_package_from_label('opaque-kappa 4 x 1,5 liter')
    assert result['article_label'] == 'opaque-kappa'
...
def test_variant_percent_and_technical_specs_are_not_package_content():
    assert extract_package_from_label('opaque-lambda 30%') is None
    assert extract_package_from_label('opaque-mu 48+') is None
    assert extract_package_from_label('opaque-nu 10000 mAh') is None
    assert extract_package_from_label('opaque-xi K150') is None
    assert extract_package_from_label('opaque-omicron 280x28x1,8 cm') is None
