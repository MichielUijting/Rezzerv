...
def append_product_candidate(
    extracted: list[dict[str, Any]], *, label: str | None, qty_raw: str | None,
    amount1_raw: str | None, amount2_raw: str | None, source_index: int,
    raw_line: str | None, normalized_line: str | None, filename: str | None,
    store_name: str | None, function_name: str, append_branch: str, parser_path: str,
    caller_line_hint: str, clean_label: CleanLabel, parse_quantity: ParseQuantity,
    parse_decimal: ParseDecimal, amount_to_float: AmountToFloat, classify_line: ClassifyLine,
    ...
    if not classification_trace:
        classification_trace = {'classification': classification, 'stage': 'runtime_gateway', 'rule': 'CLASSIFY_LINE_CALLBACK', 'matched': label_value}
    ...
    candidate_line = {
        ...
        'unit_price': amount_to_float(unit_price), 'line_total': line_total_float,
        'discount_amount': None, 'barcode': None, 'confidence_score': confidence_score,
    ...
    producer_trace = {
        'filename': filename, 'store_name': store_name, 'function_name': function_name,
        'append_branch': append_branch, 'parser_path': parser_path, 'source_index': source_index,
        'raw_line': raw_line, 'normalized_line': normalized_line, 'label': label_value,
        'raw_label': raw_label_value, 'amount': line_total_float, 'classification': classification,
        'classification_allows_append': classification_allowed, 'append_allowed': append_allowed,
        'caller_line_hint': caller_line_hint, 'classification_rule': classification_trace.get('rule'),
    ...
    if package_metadata:
        producer_trace.update({'package_extraction_applied': True, 'package_text': package_metadata.get('package_text'), 'package_quantity': package_metadata.get('package_quantity'), 'package_unit': package_metadata.get('package_unit')})
    if ah_leading_quantity_metadata:
        producer_trace.update({'ah_leading_quantity_normalization_applied': True, 'ah_leading_quantity_original_label': ah_leading_quantity_metadata.get('original_label'), 'ah_leading_quantity_normalized_label': ah_leading_quantity_metadata.get('normalized_label'), 'ah_leading_quantity': ah_leading_quantity_metadata.get('quantity'), 'ah_leading_quantity_token': ah_leading_quantity_metadata.get('token')})
    if name_metadata:
        producer_trace.update({'product_name_normalization_applied': True, 'product_name_original_label': name_metadata.get('original_label'), 'product_name_normalized_label': name_metadata.get('normalized_label'), 'product_name_normalization_rules': name_metadata.get('normalization_rules')})
    if financial_metadata:
        producer_trace.update({'line_type': financial_metadata.get('line_type'), 'is_spaarzegels': True, 'include_in_receipt_total': True, 'exclude_from_inventory': True, 'external_matching_allowed': False, 'matched_spaarzegels_term': financial_metadata.get('matched_spaarzegels_term')})
