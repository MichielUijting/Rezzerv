...
def build_receipt_line_diagnosis(
    ...
    """Build a fast, stable, read-only diagnosis report.

    R9-31F.1 deliberately keeps this Swagger report stored-only:
    - no live reparse
    - no OCR
    - no preprocessing
    - no parser rerun
    ...
    The filenames argument is kept only for backward-compatible callers and is
    ignored by design: the report covers all active receipts for the household.
    """
    start_total = _now_ms()
