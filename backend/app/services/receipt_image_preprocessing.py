...
def _normalize_receipt_axis_angle(angle: float) -> float:
    while angle <= -90.0:
        angle += 180.0
    while angle > 90.0:
        angle -= 180.0
...
def _dominant_receipt_contour_rotation_angle(rgb) -> float | None:
    ...
    for contour in sorted(contours, key=cv2.contourArea, reverse=True)[:10]:
        ...
        if aspect_ratio < 1.05:
            continue
        rotation_angle = float(rect_angle + 90.0 if rect_width > rect_height else rect_angle)
        rotation_angle = _normalize_receipt_axis_angle(rotation_angle)
        candidates.append((area_ratio, aspect_ratio, rotation_angle, rect_angle, rect_width, rect_height))
    if not candidates:
        LOGGER.warning('Receipt preprocessing CONTOUR_ANGLE skipped')
        return None
    candidates.sort(key=lambda item: item[0], reverse=True)
    area_ratio, aspect_ratio, rotation_angle, rect_angle, rect_width, rect_height = candidates[0]
    LOGGER.warning(
        'Receipt preprocessing CONTOUR_ANGLE selected rotation_angle=%.2f area_ratio=%.3f aspect=%.2f rect_angle=%.2f rect_size=%.1fx%.1f',
        rotation_angle,
        area_ratio,
        aspect_ratio,
        rect_angle,
...
def _dominant_text_angle(rgb) -> float:
    ...
    for raw in lines[:, 0, :]:
        ...
        normalized_angle = _normalize_line_angle(float(np.degrees(np.arctan2(y2 - y1, x2 - x1))))
        if length >= 80 and abs(normalized_angle) <= 45:
...
def _correct_rotation(rgb):
    ...
    if contour_angle is not None:
        LOGGER.warning('Receipt preprocessing ROTATION_STAGE method=receipt_contour applied_angle=%.2f', contour_angle)
        return _rotate_keep_bounds(rgb, contour_angle), contour_angle, contour_angle, True
...
def preprocess_receipt_image_for_ocr(file_bytes: bytes) -> bytes:
    LOGGER.warning('Receipt preprocessing PIPELINE_ENTER variant=contour-rotation-v1 bytes=%s', len(file_bytes) if file_bytes else 0)
    ...
    try:
        ...
        _write_debug_png(DEBUG_ORIGINAL_PATH, original, 'contour-rotation-v1:original')
        _write_debug_png(DEBUG_ROTATED_PATH, rotated, 'contour-rotation-v1:rotated')
        _write_debug_png(DEBUG_FINAL_PATH, final, 'contour-rotation-v1:final')
        _write_debug_png(DEBUG_OUTPUT_PATH, final, 'contour-rotation-v1:ocr-input-alias')
        LOGGER.warning(
