...
def _chain_name(value: Any) -> str:
    ...
    if text_value.startswith('albertheijn'):
        return 'albertheijn'
    if text_value.startswith('jumbo'):
        return 'jumbo'
    if text_value.startswith('lidl'):
        return 'lidl'
    if text_value.startswith('plus'):
        return 'plus'
    if text_value.startswith('aldi'):
        return 'aldi'
    return text_value


def _store_matches_baseline(expected: Any, actual: Any) -> bool:
    ...
    if expected_norm == actual_norm:
        return True
    return _chain_name(expected_norm) == _chain_name(actual_norm)
...
def _score_actual_match(expected: dict[str, Any], actual: dict[str, Any]) -> tuple[int, dict[str, bool], str]:
    ...
    if _store_matches_baseline(expected.get('store_name'), actual.get('store_name')):
        score += 30
