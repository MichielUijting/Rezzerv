...
_REQUIRED_CANDIDATE_INDEX = "idx_external_product_candidates_context"
_REQUIRED_CANDIDATE_INDEX_COLUMNS = (
    "context_key",
    "retailer_code",
    "candidate_source_name",
    "candidate_source_product_code",
...
def build_candidate_context_key(
    retailer_code: str,
    ...
    return (
        f"external-preview:{normalize_candidate_text(retailer_code)}:"
        f"{normalize_candidate_text(receipt_line_text)}"
    )
...
def ensure_external_product_candidates_schema() -> None:
    """Validate the Alembic-owned external candidate schema without mutating it."""
    with engine.connect() as conn:
        inspector = inspect(conn)
        table_name = "external_product_candidates"
        if not inspector.has_table(table_name):
            raise RuntimeError(
                "Canonical external_product_candidates schema ontbreekt. "
        ...
        if missing:
            raise RuntimeError(
                "Canonical external_product_candidates schema wijkt af; ontbrekende "
        ...
        index = indexes.get(_REQUIRED_CANDIDATE_INDEX)
        actual_columns = tuple((index or {}).get("column_names") or ())
        if (
            index is None
            or actual_columns != _REQUIRED_CANDIDATE_INDEX_COLUMNS
            or bool(index.get("unique"))
        ):
            raise RuntimeError(
                "Canonical external_product_candidates index wijkt af: "
                f"{_REQUIRED_CANDIDATE_INDEX}. Voer Alembic migrations uit."
            )
...
def _candidate_identity(candidate: dict[str, Any]) -> dict[str, str]:
    return {
        ...
        "candidate_source_product_code": str(
...
def _find_existing_candidate(
    ...
    return conn.execute(
        ...
        {"context_key": context_key, "retailer_code": retailer_code, **identity},
    ).mappings().first()
...
def _is_protected(existing: dict[str, Any] | None) -> bool:
    ...
    return bool(
        ...
        or existing.get("is_user_confirmed")
        or existing.get("is_external_database_override")
    )
...
def save_matchpreview_candidates(
    ...
    normalized_retailer = str(
        match_result.get("retailer_code") or retailer_code or ""
    ).strip().lower()
    ...
    storage_purchase_import_line_id = (
        purchase_import_line_id or build_preview_import_line_fallback(context_key)
    )
    ...
    with engine.begin() as conn:
        for candidate in candidates:
            existing = _find_existing_candidate(
                conn, context_key, normalized_retailer, candidate
            )
            if _is_protected(existing):
                skipped.append(
                    {"id": existing.get("id"), "reason": "protected_candidate"}
                )
                continue

            candidate_id = str(existing.get("id")) if existing else str(uuid.uuid4())
            candidate_source_name = str(
                candidate.get("candidate_source_name") or "external_database"
            ).strip()
            candidate_source_product_code = str(
                candidate.get("candidate_source_product_code")
                or candidate.get("retailer_article_number")
            ...
            params = {
                ...
                "created_by": str(
                    ...
                    or "external_database_matchpreview_save_v1"
                ).strip(),
...
def list_saved_external_product_candidates(
    ...
    if raw_payload:
        ...
        if isinstance(payload, dict):
            metadata = (
                ...
                if isinstance(payload.get("batch_metadata"), dict)
                else {}
            )
            for value in (
...
def _m2c2h5_purchase_import_placeholder(row: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "receipt_line_text": article_name
        or str(row.get("external_article_code") or "").strip(),
        "candidate_name": "",
...
def _m2c2h5_list_purchase_import_placeholders(
    conn,
    existing_context_keys: set[str],
    limit: int,
) -> list[dict[str, Any]]:
    del existing_context_keys
    ...
    if "batch_id" in columns and _m2c2h5_table_exists(conn, "purchase_import_batches"):
        ...
        if "id" in batch_columns:
            ...
            batch_raw_payload_expr = _m2c2h5_col(
                "pib", batch_columns, ["raw_payload"], "''"
            )
    ...
    global_product_expr = _m2c2h5_col(
...
def _m2c2l_enrich_linked_receipt_items(
    ...
    rows = conn.execute(
        text(
            ...
            f"pig.inventory_group_key AS linked_product_type_id, "
            f"pig.display_name AS linked_product_type, "
            f"pgm.confirmed_by_user AS confirmed_by_user, "
    ...
    for placeholder in placeholders:
        ...
        if linked:
            next_placeholder["linked_candidate_name"] = str(
                linked.get("linked_candidate_name") or ""
            ).strip()
            next_placeholder["linked_gtin"] = str(linked.get("linked_gtin") or "").strip()
            next_placeholder["linked_product_type_id"] = str(
                linked.get("linked_product_type_id") or ""
            ).strip()
            next_placeholder["linked_product_type"] = str(
                linked.get("linked_product_type") or ""
            ).strip()
...
def _m2c2i_fix7a3_normalized_receipt_key(
    retailer_code: Any,
    receipt_line_text: Any,
) -> str:
    retailer = str(retailer_code or "").strip().lower()
    text_value = str(receipt_line_text or "").strip().lower().replace(".", "")
...
def _m2c2i_fix7b_best_candidate_brand(candidate: dict[str, Any]) -> str | None:
    return (
...
def _m2c2i_fix7b_candidate_is_confirmed_catalog_link(
    candidate: dict[str, Any],
) -> bool:
    status_values = {
    ...
    return bool(
        ...
        or candidate.get("is_user_confirmed")
        or candidate.get("is_external_database_override")
    )
...
def _m2c2i_fix7b_create_or_reuse_catalog_product_for_candidate(
    conn,
    candidate: dict[str, Any],
) -> str:
    if not _m2c2h5_table_exists(conn, "global_products") or not _m2c2h5_table_exists(
        conn, "product_identities"
    ):
        ...
        conn.execute(
            text(
                ...
                INSERT INTO product_identities (
                    ...
                    source, confidence_score, is_primary, created_at, updated_at,
                    global_product_id
                )
                SELECT
                    :id, '', 'retailer_article_number', :identity_value,
                    :source, 1.0, TRUE, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP,
                    :global_product_id
                WHERE NOT EXISTS (
                    SELECT 1
                    FROM product_identities
...
def _m2c2i_fix7b_ensure_catalog_products_for_article_codes(
    conn,
    ...
    for candidate in candidates:
        ...
        if not source_code or not _m2c2i_fix7b_candidate_is_confirmed_catalog_link(
            next_candidate
        ):
            enriched.append(next_candidate)
            continue

        global_product_id = _m2c2i_fix7b_create_or_reuse_catalog_product_for_candidate(
            conn, next_candidate
        )
        if global_product_id:
            ...
            next_candidate["is_user_confirmed"] = True
        enriched.append(next_candidate)
    return enriched
...
def _m2c2i_fix7b_dedupe_top_receipt_items(
    ...
    for item in items:
        key = _m2c2i_fix7a3_normalized_receipt_key(
            item.get("retailer_code"), item.get("receipt_line_text")
        )
    ...
    def row_score(item: dict[str, Any]) -> tuple[Any, ...]:
        status = str(
            item.get("status") or item.get("candidate_status") or ""
        ).strip()
        return (
            status_priority.get(status, 0),
            1 if str(item.get("global_product_id") or "").strip() else 0,
            1 if str(item.get("matched_household_article_id") or "").strip() else 0,
    ...
    for entries in grouped.values():
        ...
        linked_entries = [
            ...
            if str(entry.get("global_product_id") or "").strip()
            and str(entry.get("status") or entry.get("candidate_status") or "").strip()
            == "linked_to_catalog"
        ]
        if linked_entries:
...
def _m2c2i_fix7c_same_retailer_for_projection(
    ...
    return bool(
        placeholder_retailer
        and candidate_retailer
        and placeholder_retailer == candidate_retailer
    )


def _m2c2i_fix2_truthy(value: Any) -> bool:
    normalized = str(value if value is not None else "").strip().lower()
    return bool(
        normalized
        and normalized not in {"-", "0", "none", "null", "undefined", "false"}
    )


def _m2c2i_fix2_has_catalog_reference(row: dict[str, Any]) -> bool:
    return any(
...
def _m2c2i_fix7a_apply_identifier_contract(row: dict[str, Any]) -> dict[str, Any]:
    next_row = dict(row)
    next_row["candidate_id"] = _m2c2i_fix7a_first_truthy(next_row, ("id",))
    next_row["external_source_name"] = _m2c2i_fix7a_first_truthy(
        next_row, ("candidate_source_name", "source_name")
    )
...
def _m2c2i_fix2_has_external_candidate_identity(row: dict[str, Any]) -> bool:
    return bool(
...
def _m2c2i_fix2_has_explicit_link_status(row: dict[str, Any]) -> bool:
    status_values = {
    ...
    return bool(
        "linked_to_catalog" in status_values
        or "user_confirmed" in status_values
        or row.get("is_user_confirmed")
        or row.get("is_external_database_override")
    )


def _m2c2i_fix2_link_priority(
    row: dict[str, Any],
    index: int,
) -> tuple[Any, ...]:
    status = str(
        row.get("candidate_status") or row.get("status") or ""
    ).strip().lower()
    return (
...
def _m2c2i_fix7a3_apply_catalog_status_to_placeholders(
    ...
    def candidate_priority(candidate: dict[str, Any]) -> tuple[Any, ...]:
        status = str(
            candidate.get("status") or candidate.get("candidate_status") or ""
        ).strip().lower()
        return (
            0 if status == "linked_to_catalog" else 1,
    ...
    def normalize_candidate_for_detail(candidate: dict[str, Any]) -> dict[str, Any]:
        next_candidate = _m2c2i_fix7a_apply_identifier_contract(dict(candidate))
        status = str(
            next_candidate.get("status") or next_candidate.get("candidate_status") or ""
        ).strip().lower()
        has_catalog = _m2c2i_fix2_has_catalog_reference(next_candidate)
        ...
        next_candidate["is_linkable_to_catalog"] = bool(
            not is_linked and _m2c2i_fix7b_catalog_source_code(next_candidate)
        )
        if is_linked:
            next_candidate["status"] = "linked_to_catalog"
            next_candidate["candidate_status"] = "linked_to_catalog"
    ...
    for candidate in candidates:
        purchase_import_line_id = str(
            candidate.get("purchase_import_line_id") or ""
        ).strip()
        if purchase_import_line_id and not purchase_import_line_id.startswith("preview:"):
            by_purchase_import_line_id.setdefault(
                purchase_import_line_id, []
            ).append(candidate)
        key = _m2c2i_fix7a3_normalized_receipt_key(
            candidate.get("retailer_code"), candidate.get("receipt_line_text")
        )
        if key and key != "|":
    ...
    for raw_placeholder in placeholders:
        placeholder = dict(raw_placeholder)
        matching_candidates: list[dict[str, Any]] = []
        purchase_import_line_id = str(
            placeholder.get("purchase_import_line_id") or ""
        ).strip()
        if purchase_import_line_id:
            matching_candidates.extend(
                by_purchase_import_line_id.get(purchase_import_line_id, [])
            )
        key = _m2c2i_fix7a3_normalized_receipt_key(
            placeholder.get("retailer_code"), placeholder.get("receipt_line_text")
        )
        ...
        for candidate in matching_candidates:
            ...
            if identity not in unique or candidate_priority(candidate) < candidate_priority(
                unique[identity]
            ):
        ...
        if best:
            status = str(
                best.get("status") or best.get("candidate_status") or ""
            ).strip().lower()
            has_catalog = bool(str(best.get("global_product_id") or "").strip())
            placeholder["candidates"] = [
                normalize_candidate_for_detail(candidate)
                for candidate in detail_candidates[:20]
            ]
            ...
            if status == "linked_to_catalog" and has_catalog:
                ...
                placeholder["global_product_id"] = (
                    str(best.get("global_product_id") or "").strip() or None
                )
                placeholder["is_linked_to_catalog"] = True
                placeholder["is_existing_link_for_receipt_item"] = True
                placeholder["canonical_catalog_product_id"] = placeholder[
                    "global_product_id"
                ]
                placeholder["retailer_article_number"] = (
                    str(
                        best.get("retailer_article_number")
                        or best.get("candidate_source_product_code")
                        or best.get("source_product_code")
                        or ""
                    ).strip()
...
def _base_list_external_receipt_items(limit: int = 500) -> dict[str, Any]:
    ensure_external_product_candidates_schema()
    ...
    with engine.begin() as conn:
        ...
        candidates = _m2c2i_fix7b_ensure_catalog_products_for_article_codes(
            conn, candidates
        )
        ...
        placeholders = _m2c2h5_list_purchase_import_placeholders(
            conn, existing_context_keys, normalized_limit
        )
...
def unlink_external_catalog_links(
    ...
    normalized_context_keys = [
        str(value).strip() for value in (context_keys or []) if str(value).strip()
    ]
    normalized_candidate_ids = [
        str(value).strip() for value in (candidate_ids or []) if str(value).strip()
    ]
    ...
    with engine.begin() as conn:
        ...
        for context_key in normalized_context_keys:
            if context_key.startswith("purchase-import-line:"):
                purchase_import_line_ids.append(
                    context_key.replace("purchase-import-line:", "", 1).strip()
                )

        purchase_import_line_ids = sorted(
            {value for value in purchase_import_line_ids if value}
        )
        purchase_import_line_unlinked_count = 0
        if purchase_import_line_ids and _m2c2h5_table_exists(
            conn, "purchase_import_lines"
        ):
            columns = _m2c2h5_table_columns(conn, "purchase_import_lines")
...
def ensure_external_receipt_item_candidates(
    ...
    for item in normalized_items:
        receipt_line_text = str(
            item.get("receipt_line_text") or item.get("receiptLineText") or ""
        ).strip()
        retailer_code = str(
            item.get("retailer_code") or item.get("retailerCode") or ""
        ).strip().lower()
        purchase_import_line_id = str(
            item.get("purchase_import_line_id")
            or item.get("purchaseImportLineId")
            or ""
        ).strip() or None
        receipt_line_id = str(
            item.get("receipt_line_id") or item.get("receiptLineId") or ""
        ).strip() or None
...
def _m2c2i_fix2_context_key(row: dict[str, Any]) -> str:
    return str(
        row.get("context_key")
...
def _m2c2i_fix2_is_candidate_placeholder(row: dict[str, Any]) -> bool:
    return bool(row.get("is_receipt_item_placeholder"))


def _m2c2i_fix2_apply_status_fields(
    ...
    for context_key, entries in groups.items():
        ...
        if explicit_linked:
            explicit_linked.sort(
                key=lambda item: _m2c2i_fix2_link_priority(item[1], item[0])
            )
            active_link_indices.add(explicit_linked[0][0])
    ...
    for index, row in enumerate(rows):
        ...
        is_linked = bool(
            index in active_link_indices
            or (is_placeholder and has_catalog_reference and explicit_link_status)
        )
        is_linkable = bool(
            not is_placeholder
            and not is_linked
        ...
        elif str(next_row.get("candidate_status") or "").strip().lower() in {
            "linked_to_catalog",
            "user_confirmed",
        }:
        ...
        for candidate in next_row.get("candidates") or []:
            ...
            nested_status = str(
                nested.get("status") or nested.get("candidate_status") or ""
            ).strip().lower()
            nested_has_catalog = _m2c2i_fix2_has_catalog_reference(nested)
            nested_is_linked = bool(
                nested_status == "linked_to_catalog" and nested_has_catalog
            )
            nested["is_linked_to_catalog"] = nested_is_linked
            nested["is_existing_link_for_receipt_item"] = nested_is_linked
            nested["is_linkable_to_catalog"] = bool(
                not nested_is_linked
                and _m2c2i_fix2_has_external_candidate_identity(nested)
            )
            if nested_is_linked:
...
def list_external_receipt_items(limit: int = 500) -> dict[str, Any]:
    payload = _base_list_external_receipt_items(limit=limit)
    rows = payload.get("items") or []
    next_payload = dict(payload)
    enriched_rows = _m2c2i_fix2_apply_status_fields(
        [dict(row) if hasattr(row, "items") else row for row in rows]
    )
    with engine.connect() as conn:
        centrally_projected_rows = project_central_link_truth_rows(
            conn, enriched_rows
        )
    next_payload["items"] = _m2c2i_fix7b_dedupe_top_receipt_items(
        centrally_projected_rows
    )
    next_payload["total"] = len(next_payload["items"])
    return next_payload


def promote_external_product_candidate(
    candidate_id: str,
            ...
            text(
                ...
                WHERE context_key = :context_key
                  ...
                  AND (
                    ...
                    OR COALESCE(is_user_confirmed, FALSE) IS TRUE
                    OR COALESCE(is_external_database_override, FALSE) IS TRUE
                  )
                """
            ),
    ...
    *,
    ...
    if not isinstance(product_type_assignment, dict):
        return {
            "ok": False,
            "promoted": False,
            "reason": "product_type_assignment_required",
        }
    ...
    with engine.begin() as conn:
        ...
        product_type_id = str(
            product_type_assignment.get("product_type_id") or ""
        ).strip()
        if isinstance(create_payload, dict):
            type_result = create_or_get_product_type_with_connection(
                conn,
                inventory_group_key=str(
                    create_payload.get("inventory_group_key") or ""
                ).strip()
                or None,
                display_name=str(
                    create_payload.get("canonical_name")
                    or create_payload.get("display_name")
                    or ""
                ).strip(),
                default_base_unit=str(
                    create_payload.get("base_unit")
                    or create_payload.get("default_base_unit")
                    or "stuk"
                ).strip(),
                aggregation_mode=str(
                    create_payload.get("aggregation_mode") or "count"
                ).strip(),
                source=str(
                    product_type_assignment.get("mapping_source")
                    or "user_created_during_link"
                ).strip(),
            )
            ...
            product_type_id = str(
                (type_result.get("product_type") or {}).get("inventory_group_key")
                or ""
            ).strip()
        ...
        membership = link_global_product_to_inventory_group_with_connection(
            ...
            source=str(
                product_type_assignment.get("mapping_source")
                or "user_confirmed_external_suggestion"
            ).strip(),
            confirmed_by_user=True,
        )
        if not membership.get("ok"):
            raise ValueError(
                str(membership.get("error") or "Producttype kon niet worden gekoppeld")
            )
