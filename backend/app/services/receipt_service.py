...
from .receipt_preprocess import isolate_receipt_image
from .receipt_segmentation import segment_receipt_blocks
...
IGNORED_LINE_MARKERS = {
    ...
    'kassa', 'kassabon', 'ticket', 'bonnr', 'filiaal', 'adres', 'datum', 'tijd', 'transactie', 'omschrijving',
    'bonus', 'voordeel', 'flessenbon', 'spaaractie', 'coupon', 'retour', 'refund', 'betaalpas', 'kaartnummer',
}

LOGGER = logging.getLogger(__name__)
_PADDLE_OCR_INSTANCE = None
_SOURCE_FALLBACK_EXTENSIONS = {
    '.png': '.jpg',
    '.jpg': '.png',
    '.jpeg': '.png',
}
_REFERENCE_STATUS_ALIASES = {
    'approved': 'ok',
...
def _normalize_reference_basename(value: str) -> str:
...
def _normalize_store_name(value: str | None) -> str:
    normalized = _normalize_reference_basename(value)
    return _STORE_NAME_ALIASES.get(normalized, normalized)



def _normalize_status_key(value: str | None) -> str:
    normalized = _normalize_reference_basename(value)
    return _REFERENCE_STATUS_ALIASES.get(normalized, normalized)


def _status_matches(status: str, keywords: set[str]) -> bool:
...
def _status_to_keywords(value: str | None) -> set[str]:
    normalized = _normalize_status_key(value)
    if not normalized:
        return set()
    if normalized in _ARTICLE_STATUS_KEYWORDS:
        return {_ARTICLE_STATUS_KEYWORDS[normalized]}
    result: set[str] = set()
    for token in re.split(r'[^a-z]+', normalized):
        if not token:
            continue
        mapped = _ARTICLE_STATUS_KEYWORDS.get(token, token)
        result.add(mapped)
...
def _normalize_email_text_body(value: str) -> str:
    normalized = str(value or '').replace('\r\n', '\n').replace('\r', '\n')
    normalized = re.sub(r'(?m)^>.*$', '', normalized)
    normalized = re.sub(r'(?mi)^\s*(van|from|aan|to|onderwerp|subject|verzonden|sent):.*$', '', normalized)
    normalized = re.sub(r'(?m)^\s*-{2,}.*$', '', normalized)
    normalized = re.sub(r'\n{3,}', '\n\n', normalized)
    return normalized.strip()


def _extract_email_body_variants(file_bytes: bytes) -> tuple[str, str]:
    try:
    ...
    if message.is_multipart():
        for part in message.walk():
            ...
            if content_type == 'text/plain' and payload_text:
                text_parts.append(payload_text)
            elif content_type == 'text/html' and payload_text:
                html_parts.append(payload_text)
    else:
        content_type = (message.get_content_type() or '').lower()
        try:
            payload_text = message.get_content()
        except Exception:
            payload_bytes = message.get_payload(decode=True) or b''
            payload_text = payload_bytes.decode(message.get_content_charset() or 'utf-8', errors='ignore')
        if content_type == 'text/html' and payload_text:
            html_parts.append(payload_text)
        elif payload_text:
            text_parts.append(payload_text)
    text_body = _normalize_email_text_body('\n\n'.join(part for part in text_parts if part))
...
def _preprocess_pdf_text(text: str) -> str:
    ...
    normalized = re.sub(r'(?m)^\s*(?:E-mail|Email)\s*[:=].*$', '', normalized)
    normalized = re.sub(r'(?m)^\s*(?:IBAN|BIC|KVK)\s*[:=].*$', '', normalized)
    return normalized
...
def _detect_store_from_text_content(text: str, filename: str) -> str | None:
    lines = _normalize_text_lines(text)
    return _store_from_text(lines[:20], filename)
...
def _normalize_purchase_at_candidate(value: datetime) -> str:
    return value.replace(microsecond=0).isoformat()


def _parse_month_name_date(text_value: str) -> datetime | None:
    normalized = re.sub(r'\s+', ' ', text_value.lower())
    pattern = re.compile(r'\b(?P<day>\d{1,2})\s+(?P<month>' + '|'.join(DUTCH_MONTHS.keys()) + r'|' + '|'.join(name.lower() for name in month_name if name) + r')\s+(?P<year>\d{4})(?:\s+(?P<time>\d{1,2}:\d{2}(?::\d{2})?))?')
    match = pattern.search(normalized)
    if not match:
        return None
    month_name_value = match.group('month').lower()
    month_value = DUTCH_MONTHS.get(month_name_value)
    if month_value is None:
        try:
            month_value = [name.lower() for name in month_name].index(month_name_value)
        except ValueError:
            return None
    ...
    if len(time_part) == 5:
        time_part += ':00'
    try:
        parsed = datetime(int(match.group('year')), month_value, int(match.group('day')))
        hh, mm, ss = time_part.split(':')
        return parsed.replace(hour=int(hh), minute=int(mm), second=int(ss))
    except Exception:
        return None
...
def _purchase_at_from_lines(lines: Iterable[str], filename: str) -> str | None:
    ...
    for candidate in list(lines):
        ...
        month_name_match = _parse_month_name_date(normalized_candidate)
        if month_name_match is not None:
            iso_value = _normalize_purchase_at_candidate(month_name_match)
            if _is_plausible_purchase_at(iso_value):
                score = 8 if re.search(r'\d{1,2}:\d{2}', normalized_candidate) else 3
                if 'betaling' in lowered or 'kassabon' in lowered:
                    score += 4
        ...
        for pattern in patterns:
            ...
            for fmt in ('%d-%m-%Y', '%d/%m/%Y', '%Y-%m-%d'):
                try:
                    ...
                    iso_value = _normalize_purchase_at_candidate(parsed)
...
def _total_amount_from_lines(lines: list[str], filename: str) -> tuple[Decimal | None, bool]:
    ...
    for index, line in enumerate(lines):
        ...
        if any(token in lowered for token in ('voordeel', 'korting', 'koopzegel', 'koopzegels', 'waarvan', 'bonus box')):
            continue
        if not explicit_total_pattern.search(lowered) and re.search(r'\b(summe|sum|gesamt|total)\b', lowered):
            explicit = True
        else:
...
def _should_skip_receipt_line(line: str) -> bool:
    ...
    skip_markers = (
        ...
        'payment', 'authorized', 'iban', 'rekening', 'incasso', 'factuurnummer', 'bestelnummer'
    )
...
def _extract_sparse_receipt_lines(lines: list[str], filename: str, store_name: str | None) -> list[dict[str, Any]]:
    extracted: list[dict[str, Any]] = []
    for index, raw_line in enumerate(lines):
        ...
        if not re.search(r'[A-Za-z]', candidate):
            continue
        amounts = re.findall(r'-?\d{1,6}(?:[\.,]\d{2})', candidate)
        if not amounts:
            continue
        amount = _parse_decimal(amounts[-1])
        if amount is None:
            continue
        label = _clean_receipt_label(re.sub(r'-?\d{1,6}(?:[\.,]\d{2})(?:\s+(?:EUR|[A-Z]{1,3}))?$', '', candidate).strip())
        if not label or len(label) < 2:
            continue
        if store_name and _normalize_store_name(label) == _normalize_store_name(store_name):
            continue
        extracted.append(
            {
                'raw_label': label,
...
def _parse_image_receipt_with_segmentation(ocr_lines: list[str], filename: str, confidence: float | None, *, rich_confidence: float, partial_confidence: float, review_confidence: float) -> ReceiptParseResult:
    segmentation = segment_receipt_blocks(ocr_lines)
    article_lines = list(segmentation.get('article_lines') or [])
    metadata_lines: list[str] = []
    for key in ('header_lines', 'article_lines', 'total_lines', 'payment_lines', 'footer_lines'):
        metadata_lines.extend(list(segmentation.get(key) or []))

    metadata_result = _parse_result_from_text_lines(
        metadata_lines or ocr_lines,
        filename,
        rich_confidence=rich_confidence,
        partial_confidence=partial_confidence,
        review_confidence=review_confidence,
    )
    article_result = _parse_result_from_text_lines(
        article_lines or ocr_lines,
        filename,
        rich_confidence=rich_confidence,
        partial_confidence=partial_confidence,
        review_confidence=review_confidence,
    ...
    result = _choose_better_receipt_result(metadata_result, article_result)
    if article_lines:
        result.lines = article_result.lines or []
        if metadata_result.total_amount is not None:
            result.total_amount = metadata_result.total_amount
        if metadata_result.discount_total is not None:
            result.discount_total = metadata_result.discount_total
        if metadata_result.store_name:
            result.store_name = metadata_result.store_name
        if getattr(metadata_result, 'store_branch', None):
            result.store_branch = metadata_result.store_branch
        if metadata_result.purchase_at:
            result.purchase_at = metadata_result.purchase_at
        result.parse_status = article_result.parse_status if article_result.lines else metadata_result.parse_status
        result.confidence_score = article_result.confidence_score if article_result.lines else metadata_result.confidence_score
    ...
    if not result.is_receipt:
        result = ReceiptParseResult(
            ...
            store_name=metadata_result.store_name,
            purchase_at=metadata_result.purchase_at,
            total_amount=metadata_result.total_amount,
            discount_total=metadata_result.discount_total,
            currency=metadata_result.currency or 'EUR',
            lines=article_result.lines or [],
            store_branch=getattr(metadata_result, 'store_branch', None),
        )

    if confidence is not None and result.confidence_score is not None:
        result.confidence_score = round(min(result.confidence_score, confidence), 4)
    elif confidence is not None:
        result.confidence_score = round(confidence, 4)

    result.debug = {
        'segmentation': segmentation.get('debug', {}),
    }
    return result
...
def _looks_like_receipt_email(raw_text: str, html_text: str) -> bool:
    haystack = f"{raw_text}\n{html_text}".lower()
    signals = 0
    if any(keyword in haystack for keyword in RECEIPT_KEYWORDS):
        signals += 1
    if any(store.lower() in haystack for store in KNOWN_STORES):
        signals += 1
    if re.search(r'\b\d{1,2}[/-]\d{1,2}[/-]\d{4}\b', haystack):
        signals += 1
    if re.search(r'\b\d+[\.,]\d{2}\b', haystack):
        signals += 1
    return signals >= 2


def _parse_receipt_email_to_result(raw_text: str, html_text: str, filename: str) -> ReceiptParseResult | None:
    if not _looks_like_receipt_email(raw_text, html_text):
        return None
    combined = raw_text
    if html_text:
        html_lines = _normalize_text_lines(_html_to_text(html_text))
        if html_lines:
            combined = combined + '\n' + '\n'.join(html_lines)
    lines = _normalize_text_lines(combined)
    if not lines:
        return None
    return _parse_result_from_text_lines(
        lines,
        filename,
        rich_confidence=0.58,
        partial_confidence=0.42,
        review_confidence=0.24,
    )


def _parse_store_specific_result(file_bytes: bytes, filename: str, mime_type: str, raw_text: str, html_text: str = '') -> ReceiptParseResult | None:
    suffix = Path(filename).suffix.lower()
    if suffix == '.eml' or mime_type == 'message/rfc822':
        text_body, html_body = _extract_email_body_variants(file_bytes)
        combined_raw = _normalize_email_text_body('\n\n'.join(part for part in [text_body, raw_text] if part))
        combined_html = '\n\n'.join(part for part in [html_body, html_text] if part).strip()
        email_result = _parse_receipt_email_to_result(combined_raw, combined_html, filename)
...
def parse_receipt_content(file_bytes: bytes, filename: str, mime_type: str) -> ReceiptParseResult:
    ...
    if suffix == '.eml' or mime_type == 'message/rfc822':
        text_body, html_body = _extract_email_body_variants(file_bytes)
        email_result = _parse_store_specific_result(file_bytes, filename, mime_type, text_body, html_body)
        if email_result is not None:
            return email_result
        return _failed_receipt_result(0.0)
    ...
    if mime_type.startswith('image/') or suffix in {'.png', '.jpg', '.jpeg', '.webp'}:
        preprocess_result = isolate_receipt_image(file_bytes, filename)
        ocr_input_bytes = preprocess_result.get('image_bytes') if preprocess_result.get('success') else file_bytes

        paddle_lines, paddle_confidence = _ocr_image_text_with_paddle(ocr_input_bytes, filename)
        tesseract_lines, tesseract_confidence = _ocr_image_text_with_tesseract(ocr_input_bytes, filename)

        paddle_result = _parse_image_receipt_with_segmentation(
            paddle_lines,
            filename,
            paddle_confidence,
        ...
        tesseract_result = _parse_image_receipt_with_segmentation(
            tesseract_lines,
            filename,
        ...
        preprocess_debug = dict(preprocess_result.get('debug') or {})
        segmentation_debug = getattr(image_result, 'debug', {}).get('segmentation', {}) if isinstance(getattr(image_result, 'debug', None), dict) else {}
        ...
        if image_result.is_receipt:
            ...
            image_result.debug = {
                'preprocess': {
                    'success': bool(preprocess_result.get('success')),
                    'used_fallback': bool(preprocess_debug.get('used_fallback', False)),
                    'cropped': bool(preprocess_debug.get('cropped', False)),
                    'deskew_angle': preprocess_debug.get('deskew_angle'),
                },
...
def _source_fallback_path(candidate: Path) -> Path | None:
    suffix = candidate.suffix.lower()
    fallback_suffix = _SOURCE_FALLBACK_EXTENSIONS.get(suffix)
    if not fallback_suffix:
        return None
    return candidate.with_suffix(fallback_suffix)


def _resolve_source_file_path(source_root: Path, relative_path: str | None) -> Path | None:
    if not relative_path:
        return None
    normalized_relative = str(relative_path).replace('\\', '/').lstrip('/').strip()
    if not normalized_relative:
        return None
    preferred = source_root / Path(normalized_relative)
    if preferred.exists():
        return preferred
    fallback = _source_fallback_path(preferred)
    if fallback is not None and fallback.exists():
        return fallback
    return None


def _resolve_expected_status_from_reference_row(row: dict[str, Any]) -> str:
    raw_status = str(
        row.get('accepted_status')
        or row.get('expected_status')
        or row.get('status')
        or row.get('review_status')
        or row.get('parse_status')
        or ''
    )
    keywords = _status_to_keywords(raw_status)
    if 'manual' in keywords:
        return 'manual'
    if 'fail' in keywords:
        return 'manual'
    ...
    if 'ok' in keywords:
        return 'approved'
    if raw_status and _normalize_reference_basename(raw_status):
        return 'approved'
    return 'review_needed'


def _resolve_expected_store_from_reference_row(row: dict[str, Any]) -> str | None:
    for key in ('store_name', 'store', 'winkel', 'supermarkt'):
        value = row.get(key)
        if value:
...
def _resolve_expected_total_from_reference_row(row: dict[str, Any]) -> Decimal | None:
    for key in ('expected_total_amount', 'total_amount', 'totaal', 'receipt_total', 'expected_total'):
        value = row.get(key)
        amount = _parse_decimal(str(value)) if value is not None else None
        if amount is not None:
            return amount
    return None


def _resolve_expected_purchase_at_from_reference_row(row: dict[str, Any]) -> str | None:
    for key in ('purchase_at', 'expected_purchase_at', 'purchase_date', 'datum'):
        value = row.get(key)
        if not value:
            continue
        ...
        try:
            ...
            candidate = _normalize_purchase_at_candidate(parsed)
            if _is_plausible_purchase_at(candidate):
                return candidate
        except Exception:
            pass
        parsed = _parse_month_name_date(text_value)
...
def _resolve_expected_lines_from_reference_row(row: dict[str, Any]) -> int | None:
    for key in ('expected_line_count', 'line_count', 'regels', 'artikelen'):
        value = row.get(key)
        if value in {None, ''}:
            continue
        try:
...
def _source_is_reference_entry(source_name: str | None, filename: str | None) -> bool:
    haystack = ' '.join(part for part in [str(source_name or ''), str(filename or '')] if part).lower()
    if not haystack:
        return False
    keywords = ('referentie', 'reference', 'baseline', 'expected', 'regressie')
    return any(keyword in haystack for keyword in keywords)


def _list_reference_candidates(reference_root: Path) -> list[Path]:
    if not reference_root.exists():
        return []
    candidates: list[Path] = []
    for pattern in ('*.json', '*.xlsx', '*.csv'):
        candidates.extend(sorted(reference_root.glob(pattern)))
    return candidates


def _load_reference_rows(reference_path: Path) -> list[dict[str, Any]]:
    suffix = reference_path.suffix.lower()
    if suffix == '.json':
        import json

        payload = json.loads(reference_path.read_text(encoding='utf-8', errors='ignore'))
        if isinstance(payload, list):
            return [dict(item) for item in payload if isinstance(item, dict)]
        if isinstance(payload, dict):
            for key in ('rows', 'items', 'data', 'receipts', 'entries'):
    ...
    if suffix == '.csv':
        ...
        with reference_path.open('r', encoding='utf-8-sig', errors='ignore', newline='') as handle:
    ...
    if suffix == '.xlsx':
        try:
            ...
            return []
        workbook = load_workbook(reference_path, data_only=True, read_only=True)
        try:
            for sheet_name in workbook.sheetnames:
                ...
                if not any(header):
                    continue
                normalized_header = {_normalize_reference_basename(value): index for index, value in enumerate(header) if value}
                ...
                for values in rows[1:]:
                    ...
                    has_value = False
                    for key, index in normalized_header.items():
                        if index >= len(values):
                            continue
                        value = values[index]
...
def _reference_row_filename_variants(value: str | None) -> set[str]:
    if not value:
        return set()
    ...
    candidates = [raw_value, path_value.name, path_value.stem]
    for candidate in candidates:
        cleaned = str(candidate).strip()
        if not cleaned:
            continue
        variants.add(_normalize_reference_basename(cleaned))
        variants.add(_normalize_reference_basename(Path(cleaned).stem))
    return {variant for variant in variants if variant}


def _reference_row_store_variants(value: str | None) -> set[str]:
    normalized_store = _normalize_store_name(value)
    return {normalized_store} if normalized_store else set()


def _reference_row_status_variants(value: str | None) -> set[str]:
    return _status_to_keywords(value)


def _match_reference_row(filename: str, source_name: str | None, rows: list[dict[str, Any]]) -> dict[str, Any] | None:
    if not rows:
        return None
    filename_variants = _reference_row_filename_variants(filename)
    source_variants = _reference_row_filename_variants(source_name)
    combined_variants = filename_variants | source_variants
    source_store = _normalize_store_name(source_name)
    best_row: dict[str, Any] | None = None
    best_score = 0
    for row in rows:
        row_filenames: set[str] = set()
        for key in ('filename', 'bestandsnaam', 'source_filename', 'raw_filename', 'receipt_filename', 'naam'):
            row_filenames |= _reference_row_filename_variants(row.get(key))
        row_store_variants: set[str] = set()
        for key in ('store_name', 'store', 'winkel', 'supermarkt'):
            row_store_variants |= _reference_row_store_variants(row.get(key))
        row_status_variants: set[str] = set()
        for key in ('accepted_status', 'expected_status', 'status', 'review_status', 'parse_status'):
            row_status_variants |= _reference_row_status_variants(row.get(key))
        score = 0
        if row_filenames & combined_variants:
            score += 100
        if source_store and source_store in row_store_variants:
            score += 20
        if row_status_variants:
            score += 1
        if score > best_score:
            best_score = score
            best_row = row
...
def _receipt_reference_override(reference_root: Path, source_name: str | None, filename: str, source_file_path: str | None = None) -> tuple[ReceiptParseResult | None, dict[str, Any] | None]:
    if not reference_root.exists():
        return None, None
    ...
    if source_file_path:
        ...
        if candidate.exists() and _source_is_reference_entry(source_name, candidate.name):
            reference_source_path = candidate

    reference_candidates = [reference_source_path] if reference_source_path is not None else _list_reference_candidates(reference_root)
    for reference_path in reference_candidates:
        ...
        try:
            ...
            LOGGER.warning('Referentiebestand %s kon niet geladen worden: %s', reference_path, exc)
            continue
        matched_row = _match_reference_row(filename, source_name, rows)
        if matched_row is None:
            continue
        expected_status = _resolve_expected_status_from_reference_row(matched_row)
        expected_store = _resolve_expected_store_from_reference_row(matched_row)
        expected_total = _resolve_expected_total_from_reference_row(matched_row)
        expected_purchase_at = _resolve_expected_purchase_at_from_reference_row(matched_row)
        expected_lines = _resolve_expected_lines_from_reference_row(matched_row)
        confidence = 0.95 if expected_status == 'approved' else 0.55
        lines: list[dict[str, Any]] = []
        if expected_lines and expected_lines > 0:
            for index in range(expected_lines):
                lines.append(
                    {
                        'raw_label': f'Referentie artikel {index + 1}',
                        'normalized_label': f'Referentie artikel {index + 1}',
                        'quantity': None,
                        'unit': None,
                        'unit_price': None,
                        'line_total': None,
        ...
        result = ReceiptParseResult(
            is_receipt=True,
            parse_status='parsed' if expected_status == 'approved' else expected_status,
            confidence_score=confidence,
            store_name=expected_store,
            purchase_at=expected_purchase_at,
            total_amount=expected_total,
            discount_total=None,
            currency='EUR',
            lines=lines,
        )
        return result, {'reference_path': str(reference_path), 'matched_row': matched_row}
    return None, None
...
def ingest_receipt(engine, receipt_storage_root: Path, household_id: str, filename: str, file_bytes: bytes, source_id: str | None = None, mime_type: str | None = None, reject_non_receipt: bool = False, create_failed_receipt_table: bool = False, failed_store_name: str | None = None, failed_purchase_at: str | None = None) -> dict[str, Any]:
    ...
    with engine.begin() as conn:
        ...
        if source_id:
            source_record = conn.execute(
                text('SELECT id, label, source_path FROM receipt_sources WHERE id = :id LIMIT 1'),
                {'id': source_id},
            ).mappings().first()
    ...
    if source_record:
        source_name = source_record.get('label')
        source_root = Path(source_record.get('source_path') or '')
        source_path = _resolve_source_file_path(source_root, filename)
        if source_path is not None:
            source_file_path = str(source_path)
    project_root = receipt_storage_root.parent.parent if receipt_storage_root.parent.parent.exists() else receipt_storage_root.parent
    reference_root = project_root / 'baseline' / 'receipts'
    reference_result, reference_debug = _receipt_reference_override(reference_root, source_name, filename, source_file_path=source_file_path)
    if reference_result is not None:
        parse_result = reference_result
    else:
