...
R9-36I/V7 note:
Python resolves ``app.services.receipt_status_baseline_service`` to this package
before the sibling ``receipt_status_baseline_service.py`` module. Therefore the
active V7 PO baseline implementation must live here, and legacy v4 imports must
not be pulled from this package to avoid circular startup imports.
"""
...
def _store_chain_match(expected: dict[str, Any], actual: dict[str, Any]) -> bool:
    ...
    if expected_chain and actual_chain:
        return _normalize_text(expected_chain) == _normalize_text(actual_chain)
...
def _actual_line_total_expr(conn) -> str:
...
def _fetch_active_actual_rows(conn, household_id: str | None = None) -> list[dict[str, Any]]:
    ...
    if household_id is not None:
        household_clause = ' AND rt.household_id = :household_id'
    ...
    rows = [
        ...
        for row in conn.execute(text(f'''
            ...
            WHERE rt.deleted_at IS NULL{household_clause}
            GROUP BY rt.id, rt.raw_receipt_id, rt.household_id, rt.store_name, rt.store_chain,
                     rt.total_amount, rt.discount_total, rt.line_count, rt.parse_status,
                     rt.deleted_at, rr.original_filename
            ORDER BY rr.original_filename
        '''), params).mappings().all()
    ]
    for row in rows:
        row['store_chain'] = normalize_store_chain(row.get('store_chain') or row.get('store_name'))
...
def _fetch_archived_receipt_scope(conn, household_id: str | None = None) -> list[dict[str, Any]]:
    ...
    rows = [dict(row) for row in conn.execute(text(f'''
        SELECT rt.id AS receipt_table_id, rr.original_filename, rt.store_name, rt.store_chain,
               rt.purchase_at, rt.total_amount, rt.parse_status, rt.line_count, rt.deleted_at
        FROM receipt_tables rt
        JOIN raw_receipts rr ON rr.id = rt.raw_receipt_id
        WHERE rt.deleted_at IS NOT NULL{household_clause}
        ORDER BY rt.deleted_at DESC, rt.created_at DESC
    '''), params).mappings().all()]
    for row in rows:
        row['store_chain'] = normalize_store_chain(row.get('store_chain') or row.get('store_name'))
    return rows
...
def _score_actual_match(expected: dict[str, Any], actual: dict[str, Any]) -> tuple[int, dict[str, bool], str]:
    ...
    if _store_chain_match(expected, actual):
        score += 30
        flags['store_chain_match'] = True
        reasons.append('winkelketen komt overeen')
...
def _reason(criteria: dict[str, Any]) -> str:
    ...
    labels = {
        ...
        'ARTICLE_COUNT_MISMATCH': 'artikelcount wijkt af van baseline',
...
def validate_receipt_status_baseline(conn, household_id: str | None = None) -> dict[str, Any]:
    actual_rows = _fetch_active_actual_rows(conn, household_id=household_id)
    ...
    for expected in expected_rows:
        ...
        details.append({
            ...
            'expected_store_chain': criteria.get('expected_store_chain'),
            'store_name': best_actual.get('store_name'),
            'store_chain': criteria.get('actual_store_chain'),
            ...
            'reason': _reason(criteria),
            'difference_reason': _reason(criteria),
    ...
    for actual in remaining_actual:
        ...
        details.append({
            ...
            'store_chain': actual.get('store_chain'),
            'technical_parse_status': technical_parse_status,
            'technical_parse_status_label': _status_label(technical_parse_status),
            'actual_parse_status': technical_parse_status,
            ...
            'reason': 'Controle nodig: actieve receipt bestaat wel in database maar niet in de baseline.',
        })
    status_counts = Counter(item.get('po_norm_status_label') for item in details if item.get('po_norm_status_label'))
    technical_counts = Counter(item.get('technical_parse_status_label') for item in details if item.get('technical_parse_status_label'))
    summary = {
        ...
        'current_backend_status_counts': dict(status_counts),
        'technical_parse_status_counts': dict(technical_counts),
        'verschil': _count_difference(dict(status_counts), dict(status_counts)),
        'failed_criteria_counts': dict(sorted(failed_counts.items())),
    }
    return {
        'runtime_datastore': get_runtime_datastore_info(),
        'policy_source': 'receipt_status_baseline_service',
        ...
        'po_norm': {
            ...
            'article_description_affects_status': False,
            'baseline_status_is_not_used_as_override': True,
            'dev_fallback_baseline_used': False,
        },
        'summary': summary,
        'details': details,
