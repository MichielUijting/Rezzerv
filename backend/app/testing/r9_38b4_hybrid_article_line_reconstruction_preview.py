...
OUTPUT_ROOT = Path('/tmp/rezzerv_raw_ocr_diagnostics/r9_38b4')
OUTPUT_JSON = OUTPUT_ROOT / 'hybrid_article_line_reconstruction_preview.json'
...
def _amount_value(text: Any) -> float | None:
    raw = _norm(text).replace('€', '').replace('C', '').replace('E', '').replace('EUR', '').strip()
    raw = raw.replace(',', '.')
    try:
...
def _detect_article_block(fragments: list[dict[str, Any]]) -> tuple[float, float, list[dict[str, Any]]]:
    ...
    for item in ordered:
        ...
        if any(token in text for token in START_TOKENS):
            ...
            break
    if start_y is None:
        # Fallback: first product-like label with amount below/nearby.
        for item in ordered:
...
def _group_labels(block: list[dict[str, Any]], median_height: float) -> list[dict[str, Any]]:
    ...
    y_tolerance = max(5.0, median_height * 0.35)
    for fragment in sorted(labels, key=_sort_key):
        text = _norm(fragment.get('text'))
        y = float(fragment.get('center_y') or 0)
        # Header/noise line in article block with unreadable separators should not become product.
        if not text or text.lower().startswith('2ee'):
            continue
        best = None
        best_delta = None
        for row in rows:
...
def _assign_amounts(label_rows: list[dict[str, Any]], amounts: list[dict[str, Any]], median_height: float) -> list[dict[str, Any]]:
    rows = [{**row, 'amounts': [], 'unassigned_reason': None} for row in label_rows]
    if not rows:
        return rows
    max_delta = max(14.0, median_height * 0.58)
    for amount in sorted(amounts, key=_sort_key):
        ay = float(amount.get('center_y') or 0)
        ax = float(amount.get('min_x') or 0)
        candidates = []
        for row in rows:
            row_text = str(row.get('label') or '').lower()
            delta = abs(ay - float(row['center_y']))
            # Discount rows get priority for negative amount fragments near them.
            score = delta
            if str(amount.get('text') or '').strip().startswith('-') and any(token in row_text for token in DISCOUNT_TOKENS):
                score -= 4
            # Prefer rows with left label when attaching right-column values.
            if ax > float(row.get('max_x') or 0):
                score -= 0.75
            candidates.append((score, delta, row))
        candidates.sort(key=lambda item: item[0])
        best_score, best_delta, best_row = candidates[0]
        if best_delta <= max_delta:
            best_row['amounts'].append({
                ...
                'delta_y_to_label': round(best_delta, 3),
            })
    for row in rows:
...
def _expected_preview_hits(rows: list[dict[str, Any]]) -> dict[str, bool]:
    ...
    expectations = {
        'bami_nasi_mix': 'bami nasi mix' in ' | '.join(rendered),
        ...
        'cashewnoten': any('cashewnoten ongezout' in line and '2,05' in line for line in rendered),
...
def build_report() -> dict[str, Any]:
    ...
    reconstructed = _assign_amounts(labels, amounts, median_height)
    rows = []
    ...
    result = {
        'test': 'R9-38B4 hybrid article-line reconstruction preview',
        'read_only': True,
        ...
        'parameters': {
            ...
            'amount_pairing_max_delta_y': max(14.0, median_height * 0.58),
            'method': 'detect article block, keep label rows separate, attach amount fragments to nearest label by y-position',
        },
        ...
        'review_needed_lines': [row for row in rows if row['classification'] in ('label_without_amount', 'multi_amount_review_needed')],
